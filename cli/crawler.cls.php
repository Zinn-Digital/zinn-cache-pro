...
use ZinnCachePro\Task;
use ZinnCachePro\Crawler as Crawler2;
use WP_CLI;
...
class Crawler extends Base {
	 ...
	 *
	 *     # List all crawlers
	 *     $ wp zinn-cache-pro-crawler list
	 *
	 * @since 1.1.0
	 */
	public function list() {
		...
		$crawler_run_interval = defined('ZINN_CACHE_PRO_CRAWLER_RUN_INTERVAL') ? ZINN_CACHE_PRO_CRAWLER_RUN_INTERVAL : 600; // Specify time in seconds for the time between each run interval
		if ($crawler_run_interval > 0) {
			$recurrence = '';
			$hours      = (int) floor($crawler_run_interval / 3600);
			if ($hours) {
				if ($hours > 1) {
					$recurrence .= sprintf(__('%d hours', 'zinn-cache-pro'), $hours);
				} else {
					$recurrence .= sprintf(__('%d hour', 'zinn-cache-pro'), $hours);
				}
			}
			...
			if ($minutes) {
				$recurrence .= ' ';
				if ($minutes > 1) {
					$recurrence .= sprintf(__('%d minutes', 'zinn-cache-pro'), $minutes);
				} else {
					$recurrence .= sprintf(__('%d minute', 'zinn-cache-pro'), $minutes);
				}
			}
		}
	 ...
	 *
	 *     # Turn on 2nd crawler
	 *     $ wp zinn-cache-pro-crawler enable 2
	 *
	 * @since 1.1.0
	 ...
	 *     $ wp zinn-cache-pro-crawler disable 1
	 *
	 * @since 1.1.0
	 * @param array $args Command arguments.
	 */
	 ...
	 *
	 *     # Reset crawler position
	 *     $ wp zinn-cache-pro-crawler reset
	 *
	 * @since 1.1.0
	 */
