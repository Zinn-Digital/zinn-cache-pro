...
class Purge {
	 ...
	 *     $ wp zinn-cache-pro-purge network_list
	 */
	...
	public function all() {
		...
		if ( $purge_ret->success ) {
			WP_CLI::success( __( 'Purged All!', 'zinn-cache-pro' ) );
		} else {
	 ...
	 *     $ wp zinn-cache-pro-purge blog 2
	 *
	...
	public function blog( $args ) {
		...
		if ( $purge_ret->success ) {
			WP_CLI::success( __( 'Purged the blog!', 'zinn-cache-pro' ) );
		} else {
			WP_CLI::error( 'Something went wrong! Got ' . $purge_ret->status_code );
		}
	}
	 ...
	 *     $ wp zinn-cache-pro-purge url https://mysite.com/
	 *
	...
	public function url( $args ) {
		...
		if ( $purge_ret->success ) {
			WP_CLI::success( __( 'Purged the URL!', 'zinn-cache-pro' ) );
		} else {
			WP_CLI::error( 'Something went wrong! Got ' . $purge_ret->status_code );
		}
	}
	...
	private function purgeby( $args, $select, $callback ) {
		...
		if ( $purge_ret->success ) {
			WP_CLI::success( __( 'Purged!', 'zinn-cache-pro' ) );
		} else {
			WP_CLI::error( 'Something went wrong! Got ' . $purge_ret->status_code );
		}
	}
	 ...
	 *     $ wp zinn-cache-pro-purge tag 1 3 5
	 *
	 ...
	 *     $ wp zinn-cache-pro-purge category 1 3 5
	 *
	 * @param array $args Positional arguments (IDs).
	 */
	 ...
	 *     $ wp zinn-cache-pro-purge post_id 1 3 5
	 *
	 * @param array $args Positional arguments (IDs).
	 */
