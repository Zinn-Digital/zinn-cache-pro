...
class Doc {
	...
	public static function maybe_on_by_gm( $id ) {
		if ( apply_filters( 'zinn_cache_pro_conf', $id ) ) {
			return;
		}
		if ( ! apply_filters( 'zinn_cache_pro_conf', Base::O_GUEST ) ) {
			return;
		}
		if ( ! apply_filters( 'zinn_cache_pro_conf', Base::O_GUEST_OPTM ) ) {
			return;
		}
		...
		echo wp_kses_post(
			'⚠️ ' .
			sprintf(
				__( 'This setting is %1$s for certain qualifying requests due to %2$s!', 'zinn-cache-pro' ),
	...
	public static function crawler_affected() {
		...
		echo '⚠️ ' . esc_html__( 'This setting will regenerate crawler list and clear the disabled list!', 'zinn-cache-pro' );
		echo '</span>';
	}
	...
	public static function privacy_policy() {
		...
		$link = sprintf(
			...
			esc_html__( 'Please see %s for more details.', 'zinn-cache-pro' ),
			sprintf(
	...
	public static function learn_more( $url, $title = '', $self_tab = false, $css_class = '', $return_output = false ) {
		...
		$title     = $title ? $title : esc_html__( 'Learn More', 'zinn-cache-pro' );
	...
	public static function one_per_line( $return_output = false ) {
		$str = esc_html__( 'One per line.', 'zinn-cache-pro' );
		if ( $return_output ) {
			return $str;
		}
		echo esc_html( $str );
	...
	public static function full_or_partial_url( $string_only = false ) {
		if ( $string_only ) {
			echo esc_html__( 'Both full and partial strings can be used.', 'zinn-cache-pro' );
		} else {
			echo esc_html__( 'Both full URLs and partial strings can be used.', 'zinn-cache-pro' );
		}
	}
	...
	public static function notice_htaccess() {
		echo '<span class="litespeed-primary">';
		echo '⚠️ ' . esc_html__( 'This setting will edit the .htaccess file.', 'zinn-cache-pro' ) . ' ';
	...
	public static function queue_issues( $return_output = false ) {
		...
		$html = sprintf(
			...
			esc_html__( 'The queue is processed asynchronously. It may take time.', 'zinn-cache-pro' ),
