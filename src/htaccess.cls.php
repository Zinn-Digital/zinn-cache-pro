...
class Htaccess extends Root {
	...
	const LS_MODULE_DONOTEDIT        = '## ZINN CACHE PRO PLUGIN - Do not edit the contents of this block! ##';
	const MARKER                     = 'ZINN_CACHE_PRO';
	const MARKER_NONLS               = 'NON_ZINN_CACHE_PRO';
	...
	public function __construct() {
		...
		$frontend_htaccess = defined( 'ZINN_CACHE_PRO_CFG_HTACCESS' ) ? constant( 'ZINN_CACHE_PRO_CFG_HTACCESS' ) : false;
		if ( $frontend_htaccess && substr( $frontend_htaccess, -10 ) === '/.htaccess' ) {
			$this->frontend_htaccess = $frontend_htaccess;
		}
		$backend_htaccess = defined( 'ZINN_CACHE_PRO_CFG_HTACCESS_BACKEND' ) ? constant( 'ZINN_CACHE_PRO_CFG_HTACCESS_BACKEND' ) : false;
		...
		$this->frontend_htaccess = apply_filters( 'zinn_cache_pro_frontend_htaccess', $this->frontend_htaccess );
		$this->backend_htaccess  = apply_filters( 'zinn_cache_pro_backend_htaccess', $this->backend_htaccess );
        ...
        $this->__rewrite_general = [
            ...
            'RewriteRule ' . preg_quote(ZINN_CACHE_PRO_DATA_FOLDER) . '/debug/.*\.log$ - [F,L]', // phpcs:ignore WordPress.PHP.PregQuoteDelimiter.Missing
	...
	public function current_mobile_agents() {
		...
		if ( ! $match ) {
			Error::t( 'HTA_DNF', __( 'Mobile Agent Rules', 'zinn-cache-pro' ) );
		}
	...
	private function _generate_rules( $cfg ) {
		...
		$new_rules[] = 'RewriteCond %{QUERY_STRING} action=async_zinn_cache_pro';
		...
		$vary_cookies = apply_filters( 'zinn_cache_pro_vary_cookies', $vary_cookies ); // todo: test if response vary header can work in latest OLS, drop the above two lines.
	...
	private function _rewrite_codes_msg( $file, $rules, $marker = false ) {
		return sprintf(
			...
			__( '<p>Please add/replace the following codes into the beginning of %1$s:</p> %2$s', 'zinn-cache-pro' ),
			esc_html( $file ),
