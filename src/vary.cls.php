...
class Vary extends Root {
	...
	public function after_user_init() {
		...
		if ( Router::is_logged_in() ) {
			...
			if ( ! empty( $_GET['zinn_cache_pro_guest'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		add_action(
			...
			function () {
				...
				add_filter( 'zinn_cache_pro_can_change_vary', '__return_false' );
			}
		);
	}
	...
	private function _maybe_guest_mode() {
		if ( defined( 'ZINN_CACHE_PRO_GUEST' ) ) {
			self::debug( '👒👒 Guest mode ' . ( ZINN_CACHE_PRO_GUEST ? 'predefined' : 'turned off' ) );
			return;
		}
		...
		if ( ! empty( $_GET['zinn_cache_pro_guest_off'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return;
		}

		self::debug( '👒👒 Guest mode' );

		! defined( 'ZINN_CACHE_PRO_GUEST' ) && define( 'ZINN_CACHE_PRO_GUEST', true );

		if ( $this->conf( Base::O_GUEST_OPTM ) ) {
			! defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) && define( 'ZINN_CACHE_PRO_GUEST_OPTM', true );
		}
	}
	...
	public function check_commenter( $comments ) {
		 ...
		 * @since 2.9.5
		 */
		if ( apply_filters( 'zinn_cache_pro_vary_check_commenter_pending', true ) ) {
			$pending = false;
	...
	private function can_change_vary() {
		...
		if ( ! apply_filters( 'zinn_cache_pro_can_change_vary', true ) ) {
			self::debug( 'can_change_vary bypassed due to zinn_cache_pro_can_change_vary hook' );
			return false;
		}
	...
	public function finalize_default_vary( $uid = false ) {
		 ...
		 */
		$vary = apply_filters( 'zinn_cache_pro_vary', $vary );
	...
	private function _finalize_curr_vary_cookies( $values_json = false ) {
		...
		$cookies = apply_filters( 'zinn_cache_pro_vary_curr_cookies', $cookies );
		if ( $cookies ) {
			$cookies = array_filter( array_unique( $cookies ) );
			self::debug( 'vary cookies changed by filter zinn_cache_pro_vary_curr_cookies', $cookies );
		}
