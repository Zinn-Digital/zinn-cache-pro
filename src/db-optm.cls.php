...
class DB_Optm extends Root {
	...
	private function _db_clean( $type ) {
		if ( 'all' === $type ) {
			...
			return __( 'Clean all successfully.', 'zinn-cache-pro' );
		}
		...
		switch ( $type ) {
            ...
            $sql_postmeta_join = function ( $table ) use ( $postmeta, $posts ) {
				...
				return __( 'Clean post revisions successfully.', 'zinn-cache-pro' );

			case 'orphaned_post_meta':
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$wpdb->query( "DELETE a FROM `$wpdb->postmeta` a LEFT JOIN `$wpdb->posts` b ON b.ID=a.post_id WHERE b.ID IS NULL" );
				return __( 'Clean orphaned post meta successfully.', 'zinn-cache-pro' );

			case 'auto_draft':
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$wpdb->query( "DELETE FROM `$wpdb->posts` WHERE post_status = 'auto-draft'" );
				return __( 'Clean auto drafts successfully.', 'zinn-cache-pro' );

			case 'trash_post':
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
            $wpdb->query( "DELETE FROM `$wpdb->posts` WHERE post_status = 'trash'" );
				return __( 'Clean trashed posts and pages successfully.', 'zinn-cache-pro' );

			case 'spam_comment':
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$wpdb->query( "DELETE FROM `$wpdb->comments` WHERE comment_approved = 'spam'" );
				return __( 'Clean spam comments successfully.', 'zinn-cache-pro' );

			case 'trash_comment':
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$wpdb->query( "DELETE FROM `$wpdb->comments` WHERE comment_approved = 'trash'" );
				return __( 'Clean trashed comments successfully.', 'zinn-cache-pro' );

			case 'trackback-pingback':
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
			$wpdb->query( "DELETE FROM `$wpdb->comments` WHERE comment_type = 'trackback' OR comment_type = 'pingback'" );
				return __( 'Clean trackbacks and pingbacks successfully.', 'zinn-cache-pro' );
			...
			if ( ! empty( $keys_to_delete ) ) {
				...
				return __( 'Clean expired transients successfully.', 'zinn-cache-pro' );
			...
			$wpdb->query(
				...
				return __( 'Clean all transients successfully.', 'zinn-cache-pro' );
			...
			if ( $result ) {
				...
				return __( 'Optimized all tables.', 'zinn-cache-pro' );
		}
	}
	...
	private function _conv_innodb() {
		...
		$tb_param = isset( $_GET['zinn_cache_pro_tb'] ) ? sanitize_text_field( wp_unslash( $_GET['zinn_cache_pro_tb'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		$msg = __( 'Converted to InnoDB successfully.', 'zinn-cache-pro' );
