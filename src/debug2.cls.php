...
class Debug2 extends Root {
	 ...
	 * Constructor.
	 *
	 * NOTE: until ZINN_CACHE_PRO_LOG is defined, calls to WP filters are not logged to
	...
	public function __construct() {
		self::$log_path_prefix = ZINN_CACHE_PRO_STATIC_DIR . '/debug/';
		...
		! defined( 'ZINN_CACHE_PRO_LOG_TAG' ) && define( 'ZINN_CACHE_PRO_LOG_TAG', get_current_blog_id() );

		if ( $this->conf( Base::O_DEBUG_LEVEL ) ) {
			! defined( 'ZINN_CACHE_PRO_LOG_MORE' ) && define( 'ZINN_CACHE_PRO_LOG_MORE', true );
		}

		defined( 'ZINN_CACHE_PRO_DEBUG_EXC_STRINGS' ) || define( 'ZINN_CACHE_PRO_DEBUG_EXC_STRINGS', $this->conf( Base::O_DEBUG_EXC_STRINGS ) );
	}
	...
	private function _maybe_init_folder() {
		...
		foreach ( $logs as $log ) {
			$old_path = ZINN_CACHE_PRO_CONTENT_DIR . '/' . $log . '.log';
	...
	public static function ended() {
		...
		$elapsed_time = number_format( ( microtime( true ) - ZINN_CACHE_PRO_TS_0 ) * 1000, 2 );
	...
	public function beta_test( $zip = false ) {
		// Removed entirely: this forged the `update_plugins` transient with a ZIP URL
		// supplied by QUIC.cloud (or by a request parameter) and ran Plugin_Upgrader on
		// it - a remote code install channel. It is a no-op now, not merely gated, and
		// the Beta Test admin tab that drove it is removed too.
		unset( $zip );
		self::debug( 'Beta test upgrade channel is removed in this build' );
		return;
	...
	public static function log_purge( $purge_header ) {
		if ( ! defined( 'ZINN_CACHE_PRO_LOG' ) && ! defined( 'ZINN_CACHE_PRO_LOG_BYPASS_NOTADMIN' ) ) {
			return;
		}
	...
	public function init() {
		...
		if ( Base::VAL_ON2 === $debug ) {
			if ( ! $this->cls( 'Router' )->is_admin_ip() ) {
				defined( 'ZINN_CACHE_PRO_LOG_BYPASS_NOTADMIN' ) || define( 'ZINN_CACHE_PRO_LOG_BYPASS_NOTADMIN', true );
				return;
			}
		}
		 ...
		 * This is after ZINN_CACHE_PRO_LOG_BYPASS_NOTADMIN to make `log_purge()` still work
		 *
		 * @since  3.0
		 */
		...
		if ( ! defined( 'ZINN_CACHE_PRO_LOG' ) ) {
			$this->_init_request();
			define( 'ZINN_CACHE_PRO_LOG', true );
		}
	}
	...
	private function _init_request( $log_file = null ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_LOG_MORE' ) ) {
	...
	private static function format_message( $msg ) {
		if ( ! defined( 'ZINN_CACHE_PRO_LOG_TAG' ) ) {
			return $msg . "\n";
		}
		...
		if ( ! isset( self::$_prefix ) ) {
			...
			self::$_prefix = sprintf( ' [%s %s %s] ', $addr, ZINN_CACHE_PRO_LOG_TAG, Str::rrand( 3 ) );
		}

		list( $usec, $sec ) = explode( ' ', microtime() );

		// Use gmdate to avoid tz-related warnings; apply offset if defined.
		$ts = gmdate( 'm/d/y H:i:s', (int) $sec + ( defined( 'ZINN_CACHE_PRO_TIME_OFFSET' ) ? (int) ZINN_CACHE_PRO_TIME_OFFSET : 0 ) );
	...
	public static function debug( $msg, $backtrace_limit = false ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_DEBUG_EXC_STRINGS' ) && Utility::str_hit_array( $msg, ZINN_CACHE_PRO_DEBUG_EXC_STRINGS ) ) {
			return;
		}
	...
	private static function push( $msg, $backtrace_limit = false ) {
		if ( defined( 'ZINN_CACHE_PRO_LOG_MORE' ) && false !== $backtrace_limit ) {
	...
	private function _download_log() {
		...
		if ( ! in_array( $log_type, $valid_logs, true ) ) {
			wp_die( esc_html__( 'Invalid log type.', 'zinn-cache-pro' ) );
		}

		$file = $this->path( $log_type );
		if ( ! file_exists( $file ) ) {
			wp_die( esc_html__( 'Log file not found.', 'zinn-cache-pro' ) );
		}

		$filename = 'zinn-cache-pro-' . $log_type . '-' . gmdate( 'Y-m-d_His' ) . '.log';
