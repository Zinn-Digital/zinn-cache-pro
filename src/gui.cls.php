...
class GUI extends Base {
	 ...
	 *
	 * Format: [ file_tag => [ days, zinn_cache_pro_only ], ... ]
	 *
	...
	public function init() {
		...
		add_filter( 'zinn_cache_pro_buffer_finalize', [ $this, 'finalize' ], 8 );
	}
	...
	public static function print_loading( $counter, $type ) {
		...
		echo "<img width='35' src='" . esc_url( ZINN_CACHE_PRO_PLUGIN_URL . 'assets/img/zinn-cache-pro.icon.svg' ) . "' alt='' />   ";
		printf(
			/* translators: 1: number, 2: text */
			esc_html__( '%1$s %2$s files left in queue', 'zinn-cache-pro' ),
			esc_html( number_format_i18n( $counter ) ),
			esc_html( $type )
		);
		echo '<p><a href="' . esc_url( admin_url( 'admin.php?page=zinn-cache-pro-page_optm' ) ) . '">' . esc_html__( 'Cancel', 'zinn-cache-pro' ) . '</a></p>';
		echo '</div>';
	}
	...
	public static function display_tab_list( $tabs ) {
		...
		foreach ( $tabs as $k => $val ) {
			$accesskey = $i <= 9 ? $i : '';
			printf(
				'<a class="litespeed-tab nav-tab" href="#%1$s" data-zinn-cache-pro-tab="%1$s" litespeed-accesskey="%2$s">%3$s</a>',
				esc_attr( $k ),
				esc_attr( $accesskey ),
				esc_html( $val )
			);
	...
	public static function dismiss() {
		...
		switch ( Router::verify_type() ) {
            ...
            defined( 'ZINN_CACHE_PRO_LOG' ) && self::debug( 'Dismiss promo ' . $promo_tag );
	 ...
	 */
	private function _is_zinn_cache_pro_page() {
		if (
	...
	public function show_promo( $check_only = false ) {
		$is_zinn_cache_pro_page = $this->_is_zinn_cache_pro_page();
		...
		if ( file_exists( ABSPATH . '.zinn_cache_pro_no_banner' ) ) {
			defined( 'ZINN_CACHE_PRO_LOG' ) && self::debug( 'Bypass banners due to silence file' );
			return false;
		}

		foreach ( $this->_promo_list as $promo_tag => $v ) {
			list( $delay_days, $zinn_cache_pro_page_only ) = $v;

			if ( $zinn_cache_pro_page_only && ! $is_zinn_cache_pro_page ) {
				continue;
			}
			...
			include ZINN_CACHE_PRO_DIR . "tpl/banner/$promo_tag.php";
			...
			defined( 'ZINN_CACHE_PRO_LOG' ) && self::debug( 'Show promo ' . $promo_tag );
	...
	public function frontend_enqueue_style_public() {
		wp_enqueue_script(
			Core::PLUGIN_NAME,
			ZINN_CACHE_PRO_PLUGIN_URL . 'assets/js/instant_click.min.js',
			[],
			Core::VER,
			[ 
				'strategy' => 'defer',
	...
	public function frontend_shortcut() {
		...
		$wp_admin_bar->add_menu(
			[
				...
				'href'  => get_admin_url( null, 'admin.php?page=zinn-cache-pro' ),
				'meta'  => [
					'tabindex' => 0,
		...
		$wp_admin_bar->add_menu(
			[
				...
				'title'  => esc_html__( 'Purge this page', 'zinn-cache-pro' ) . ' - LSCache',
		...
		if ( $this->has_cache_folder( 'ucss' ) ) {
			...
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Purge this page', 'zinn-cache-pro' ) . ' - UCSS',
		...
		$wp_admin_bar->add_menu(
			[
				...
				'title'  => esc_html__( 'Mark this page as ', 'zinn-cache-pro' ),
				'meta'   => [ 'tabindex' => '0' ],
			]
		);
		...
		if ( $current_uri ) {
			...
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Forced cacheable', 'zinn-cache-pro' ),
			...
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Non cacheable', 'zinn-cache-pro' ),
					'href'   => Utility::build_url( Router::ACTION_CONF, Conf::TYPE_SET, false, true, $append_arr ),
				]
			);
			...
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Private cache', 'zinn-cache-pro' ),
					'href'   => Utility::build_url( Router::ACTION_CONF, Conf::TYPE_SET, false, true, $append_arr ),
				]
			);
			...
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'No optimization', 'zinn-cache-pro' ),
					'href'   => Utility::build_url( Router::ACTION_CONF, Conf::TYPE_SET, false, true, $append_arr ),
				]
			);
		}
		...
		$wp_admin_bar->add_menu(
			[
				...
				'title'  => esc_html__( 'More settings', 'zinn-cache-pro' ),
				'href'   => get_admin_url( null, 'admin.php?page=zinn-cache-pro-cache' ),
			]
		);
		...
		$wp_admin_bar->add_menu(
			[
				...
				'title'  => esc_html__( 'Purge All', 'zinn-cache-pro' ) . ' - ' . esc_html__( 'LSCache', 'zinn-cache-pro' ),
		...
		if ( $this->conf( self::O_CDN_CLOUDFLARE ) ) {
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Purge All', 'zinn-cache-pro' ) . ' - ' . esc_html__( 'Cloudflare', 'zinn-cache-pro' ),
		...
		if ( $this->has_cache_folder( 'ccss' ) ) {
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Purge All', 'zinn-cache-pro' ) . ' - CCSS',
		...
		if ( $this->has_cache_folder( 'ucss' ) ) {
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Purge All', 'zinn-cache-pro' ) . ' - UCSS',
		...
		do_action( 'zinn_cache_pro_frontend_shortcut' );
	}
	...
	public function backend_shortcut() {
		...
		if ( defined( 'ZINN_CACHE_PRO_DISABLE_ALL' ) && ZINN_CACHE_PRO_DISABLE_ALL ) {
			...
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Enable All Features', 'zinn-cache-pro' ),
					'href'   => 'admin.php?page=zinn-cache-pro-toolbox#settings-debug',
					'meta'   => [ 'tabindex' => '0' ],
				]
			);
		...
		$wp_admin_bar->add_menu(
			[
				...
				'title'  => esc_html__( 'Manage', 'zinn-cache-pro' ),
				'href'   => 'admin.php?page=zinn-cache-pro',
				'meta'   => [ 'tabindex' => '0' ],
			]
		);
		...
		$wp_admin_bar->add_menu(
			[
				...
				'title'  => esc_html__( 'Settings', 'zinn-cache-pro' ),
				'href'   => 'admin.php?page=zinn-cache-pro-cache',
				'meta'   => [ 'tabindex' => '0' ],
			]
		);
		...
		if ( ! is_network_admin() ) {
			$wp_admin_bar->add_menu(
				[
					...
					'title'  => esc_html__( 'Image Optimization', 'zinn-cache-pro' ),
					'href'   => 'admin.php?page=zinn-cache-pro-img_optm',
					'meta'   => [ 'tabindex' => '0' ],
				]
			);
		}
		...
		do_action( 'zinn_cache_pro_backend_shortcut' );
	}
	...
	public static function img_optm_clean_up( $unfinished_num ) {
		return sprintf(
			...
			esc_attr__( 'Remove all previous unfinished image optimization requests.', 'zinn-cache-pro' ),
			esc_html__( 'Clean Up Unfinished Data', 'zinn-cache-pro' ) . ( $unfinished_num ? ': ' . Admin_Display::print_plural( $unfinished_num, 'image' ) : '' )
		);
	}
	...
	public static function plugin_install_link( $title, $name, $v ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		$action = sprintf(
			...
			esc_attr( $name ),
			esc_attr( $title ),
			esc_attr( sprintf( __( 'Install %s', 'zinn-cache-pro' ), $title ) ),
			esc_html__( 'Install Now', 'zinn-cache-pro' )
		);
	...
	public static function plugin_upgrade_link( $title, $name, $v ) {
		...
		$msg = sprintf(
			...
			__('<a href="%1$s" %2$s>View version %3$s details</a> or <a href="%4$s" %5$s target="_blank">update now</a>.', 'zinn-cache-pro'),
			esc_url( $details_url ),
			sprintf(
				...
				esc_attr(
					sprintf(
						...
						__( 'View %1$s version %2$s details', 'zinn-cache-pro' ),
						$title,
						$v
					)
				)
			...
			sprintf(
				...
				esc_attr(
					sprintf(
						...
						__( 'Update %s now', 'zinn-cache-pro' ),
						$title
					)
				)
			)
	...
	public function finalize( $buffer ) {
		...
		if ( $this->conf( Base::O_GUEST ) && false !== strpos( $buffer, '<head>' ) && defined( 'ZINN_CACHE_PRO_IS_HTML' ) ) {
			$buffer = $this->_enqueue_guest_docref_js( $buffer );
		}

		if ( defined( 'ZINN_CACHE_PRO_GUEST' ) && ZINN_CACHE_PRO_GUEST && false !== strpos( $buffer, '</body>' ) && defined( 'ZINN_CACHE_PRO_IS_HTML' ) ) {
	...
	private function _enqueue_guest_docref_js( $buffer ) {
		$js_con = File::read( ZINN_CACHE_PRO_DIR . self::LIB_GUEST_DOCREF_JS );
	...
	private function _enqueue_guest_js( $buffer ) {
		$js_con = File::read( ZINN_CACHE_PRO_DIR . self::LIB_GUEST_JS );
		// Build path for guest endpoint using wp_parse_url for compatibility.
		$guest_update_path = wp_parse_url( ZINN_CACHE_PRO_PLUGIN_URL . self::PHP_GUEST, PHP_URL_PATH );
		$js_con            = str_replace( 'zinn_cache_pro_url', esc_url( $guest_update_path ), $js_con );
