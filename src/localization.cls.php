...
class Localization extends Base {
	...
	public function init() {
		add_filter( 'zinn_cache_pro_buffer_finalize', [ $this, 'finalize' ], 23 ); // After page optm
	}
	...
	private function _rewrite( $url ) {
		return ZINN_CACHE_PRO_STATIC_URL . '/localres/' . $this->_filepath( $url );
	}
	...
	private function _realpath( $url ) {
		return ZINN_CACHE_PRO_STATIC_DIR . '/localres/' . $this->_filepath( $url );
	}
	...
	public function finalize( $content ) {
		...
		foreach ( $domains as $v ) {
			...
			$content = str_replace( $domain, ZINN_CACHE_PRO_STATIC_URL . '/localres/' . base64_encode( $domain ), $content ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
		}
