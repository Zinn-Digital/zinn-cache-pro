...
trait Cloud_Misc {
	...
	private function _load_qc_status_for_dash( $type, $force = false ) {
		// Removed: a third party must not control markup rendered inside our customers'
		// WP admin. Returning '' makes every dashboard/CDN panel render empty.
		unset( $type, $force );
		return '';
		...
		if ( ! empty( $data['qc_activated'] ) ) {
			...
			if ( empty( $this->_summary['qc_activated'] ) || $this->_summary['qc_activated'] !== $data['qc_activated'] ) {
				$msg = sprintf( __( 'Congratulations, %s successfully set this domain up for the online services with CDN service.', 'zinn-cache-pro' ), 'QUIC.cloud' );
	...
	public function check_dev_version() {
		...
		// Show the dev banner
		require_once ZINN_CACHE_PRO_DIR . 'tpl/banner/new_version_dev.tpl.php';
	}
	...
	public static function version_check( $src = false ) {
		// Removed: this reported the PHP version, plugin version and site URL to
		// wpapi.quic.cloud on install, upgrade and uninstall, with no option to opt out.
		// Upstream already returns null on its own bypass path, so callers are unaffected.
		unset( $src );
		return;

		$req_data = [
			'v'   => defined( 'ZINN_CACHE_PRO_CUR_V' ) ? ZINN_CACHE_PRO_CUR_V : '',
			'src' => $src,
			'php' => phpversion(),
		];
		...
		if ( defined( 'ZINN_CACHE_PRO_ERR' ) ) {
			$zinn_cache_pro_err   = constant( 'ZINN_CACHE_PRO_ERR' );
			$req_data['err'] = base64_encode( ! is_string( $zinn_cache_pro_err ) ? wp_json_encode( $zinn_cache_pro_err ) : $zinn_cache_pro_err ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode
		}
	...
	public function news() {
		// Removed: remote news/announcement content from the QUIC.cloud API.
		return;
		...
		require_once ZINN_CACHE_PRO_DIR . 'tpl/banner/cloud_news.tpl.php';
	}
	...
	public function show_promo() {
		// Removed: promo content is supplied by the QUIC.cloud service we no longer use.
		return;
		...
		require_once ZINN_CACHE_PRO_DIR . 'tpl/banner/cloud_promo.tpl.php';
	}
	...
	public function maybe_preview_banner() {
		if ( false !== strpos( $this->_cloud_server, 'preview.' ) ) {
			Admin_Display::note( __( 'Linked to QUIC.cloud preview environment, for testing purpose only.', 'zinn-cache-pro' ), true, true, 'litespeed-warning-bg' );
		}
	}
}
