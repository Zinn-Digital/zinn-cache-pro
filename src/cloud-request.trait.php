...
trait Cloud_Request {
	...
	private function _maybe_cloud( $service_tag ) {
		// The QUIC.cloud service coupling is removed from this fork. Deny every cloud
		// request unconditionally. false is upstream's own "cloud unavailable" value:
		// _get()/_post() return it verbatim and every caller already handles it.
		self::debug( 'Cloud services are disabled in this build [srv] ' . $service_tag );
		return false;
		...
		if ( preg_match( '#^(([1-9]?\d|1\d\d|25[0-5]|2[0-4]\d)\.){3}([1-9]?\d|1\d\d|25[0-5]|2[0-4]\d)$#', Utility::parse_url_safe( $site_url, PHP_URL_HOST ) ) ) {
			...
			$msg = __( 'In order to use QC services, need a real domain name, cannot use an IP.', 'zinn-cache-pro' );
			...
			if ( ! empty( $this->_summary[ $timestamp_tag . '_request.' . $service_tag ] ) ) {
				...
				if ( $expired > 0 ) {
					...
					if ( self::API_VER !== $service_tag ) {
						$msg =
							__( 'Cloud Error', 'zinn-cache-pro' ) .
							': ' .
							sprintf(
								__( 'Please try after %1$s for service %2$s.', 'zinn-cache-pro' ),
	...
	private function _qc_time_file( $service_tag, $type = 'last' ) {
		...
		$legacy_file = ZINN_CACHE_PRO_STATIC_DIR . '/qc_' . $type . '_request' . md5( $service_tag );
		if ( file_exists( $legacy_file ) ) {
			wp_delete_file( $legacy_file );
		}
		$service_tag = preg_replace( '/[^a-zA-Z0-9]/', '', $service_tag );
		return ZINN_CACHE_PRO_STATIC_DIR . '/qc.' . $type . '.' . $service_tag;
	}
	...
	private function _parse_response( $response, $service, $service_tag, $server ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( $visible_err ) {
				$msg = esc_html__( 'Failed to request via WordPress', 'zinn-cache-pro' ) . ': ' . esc_html( $error_message ) . ' [server] ' . esc_html( $server ) . ' [service] ' . esc_html( $service );
				Admin_Display::error( $msg );
		...
		if ( ! is_array( $json ) ) {
			...
			if ( $visible_err ) {
				$msg = esc_html__( 'Failed to request via WordPress', 'zinn-cache-pro' ) . ': ' . esc_html( $response['body'] ) . ' [server] ' . esc_html( $server ) . ' [service] ' . esc_html( $service );
				Admin_Display::error( $msg );
		...
		if ( ! empty( $json['_code'] ) ) {
			...
			if ( 'unpulled_images' === $json['_code'] ) {
				$msg = __( 'Cloud server refused the current request due to unpulled images. Please pull the images first.', 'zinn-cache-pro' );
				Admin_Display::error( $msg );
				return false;
			}
			if ( 'blocklisted' === $json['_code'] ) {
				$msg = __( 'Your domain_key has been temporarily blocklisted to prevent abuse. You may contact support at QUIC.cloud to learn more.', 'zinn-cache-pro' );
				Admin_Display::error( $msg );
				return false;
			}
			...
			if ( 'rate_limit' === $json['_code'] ) {
				...
				$msg = __( 'Cloud server refused the current request due to rate limiting. Please try again later.', 'zinn-cache-pro' );
				Admin_Display::error( $msg );
				return false;
			}
			...
			if ( 'heavy_load' === $json['_code'] || 'redetect_node' === $json['_code'] ) {
				...
				Admin_Display::info( __( 'Redetected node', 'zinn-cache-pro' ) . ': ' . Error::msg( $json['_code'] ) );
				$this->detect_cloud( $service, true );
			}
		}
	...
	public function extract_msg( $json, $service, $server = false, $is_callback = false ) {
		if ( ! empty( $json['_info'] ) ) {
			...
			$msg  = __( 'Message from QUIC.cloud server', 'zinn-cache-pro' ) . ': ' . $json['_info'];
		...
		if ( ! empty( $json['_note'] ) ) {
			...
			$msg  = __( 'Message from QUIC.cloud server', 'zinn-cache-pro' ) . ': ' . $json['_note'];
			$msg .= $this->_parse_link( $json );
		...
		if ( ! empty( $json['_success'] ) ) {
			...
			$msg  = __( 'Good news from QUIC.cloud server', 'zinn-cache-pro' ) . ': ' . $json['_success'];
			$msg .= $this->_parse_link( $json );
		...
		if ( ! empty( $json['_err_req_v'] ) ) {
			...
			$msg = sprintf( __( '%1$s plugin version %2$s required for this action.', 'zinn-cache-pro' ), Core::NAME, 'v' . $json['_err_req_v'] . '+' ) .
		...
		if ( ! $is_callback && ( empty( $json['_res'] ) || 'ok' !== $json['_res'] ) ) {
			...
			$msg            = __( 'Failed to communicate with QUIC.cloud server', 'zinn-cache-pro' ) . ': ' . $str_translated . ' [server] ' . esc_html( $server ) . ' [service] ' . esc_html( $service );
			$msg           .= $this->_parse_link( $json );
			$visible_err    = self::API_VER !== $service && self::API_NEWS !== $service && self::SVC_D_DASH !== $service;
			if ( $visible_err ) {
				Admin_Display::error( $msg );
