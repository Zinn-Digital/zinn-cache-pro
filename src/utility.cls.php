...
class Utility extends Root {
	...
	public static function ping( $domain ) {
		...
		// Removed: the raw fsockopen() probe that ranked QUIC.cloud nodes by latency.
		// 99999 is upstream's own "unreachable" value, so the int contract is unchanged.
		$status = 99999;
	...
	public static function readable_time( $seconds_or_timestamp, $timeout = 3600, $forward = false ) {
		if ( 10 === strlen( (string) $seconds_or_timestamp ) ) {
			...
			if ( $seconds > $timeout ) {
				return gmdate( 'm/d/Y H:i:s', (int) $seconds_or_timestamp + (int) ZINN_CACHE_PRO_TIME_OFFSET );
			}
		} else {
		...
		if ( ! $res ) {
			return $forward ? __( 'right now', 'zinn-cache-pro' ) : __( 'just now', 'zinn-cache-pro' );
		}

		return $forward ? $res : sprintf( __( ' %s ago', 'zinn-cache-pro' ), $res );
	}
	...
	public static function compatibility() {
		require_once ZINN_CACHE_PRO_DIR . 'lib/php-compatibility.func.php';
	}
	...
	public static function att_short_path( $url ) {
		if ( ! defined( 'ZINN_CACHE_PRO_UPLOAD_PATH' ) ) {
			$_wp_upload_dir = wp_upload_dir();

			$upload_path = self::url2uri( $_wp_upload_dir['baseurl'] );

			define( 'ZINN_CACHE_PRO_UPLOAD_PATH', $upload_path );
		}

		$local_file = self::url2uri( $url );

		$short_path = substr( $local_file, strlen( ZINN_CACHE_PRO_UPLOAD_PATH ) + 1 );
	...
	public static function make_relative( $url ) {
		self::domain_const();
		if ( 0 === strpos( $url, ZINN_CACHE_PRO_DOMAIN ) ) {
			$url = substr( $url, strlen( ZINN_CACHE_PRO_DOMAIN ) );
		}
		return trim( $url );
	}
	...
	/**
	 * Define ZINN_CACHE_PRO_DOMAIN using the home URL (no trailing slash).
	 *
	 * @since 1.3
	 * @return void
	 */
	public static function domain_const() {
		if ( defined( 'ZINN_CACHE_PRO_DOMAIN' ) ) {
			return;
		}
		...
		define( 'ZINN_CACHE_PRO_DOMAIN', $domain );
	}
	...
	public static function build_url( $action, $type = false, $is_ajax = false, $page = null, $append_arr = [], $unescape = false ) {
		...
		if ( '_ori' === $page ) {
			$page                         = true;
			$append_arr['_zinn_cache_pro_ori'] = 1;
		}
		...
		if ( ! $is_ajax ) {
			...
			$combined = 'admin-ajax.php?action=zinn_cache_pro_ajax&' . Router::ACTION . '=' . $action;
		}
	...
	public static function internal( $host ) {
		if ( ! defined( 'ZINN_CACHE_PRO_FRONTEND_HOST' ) ) {
			if ( defined( 'WP_HOME' ) ) {
				$home_host = constant( 'WP_HOME' );
			} else {
				$home_host = get_option( 'home' );
			}
			define( 'ZINN_CACHE_PRO_FRONTEND_HOST', (string) wp_parse_url( $home_host, PHP_URL_HOST ) );
		}

		if ( ZINN_CACHE_PRO_FRONTEND_HOST === $host ) {
			return true;
		}

		if ( ! isset( self::$_internal_domains ) ) {
			self::$_internal_domains = apply_filters( 'zinn_cache_pro_internal_domains', [] );
		}
	...
	public static function is_internal_file( $url, $addition_postfix = false ) {
		...
		if ( '/' === substr( $url_parsed['path'], 0, 1 ) ) {
			...
			if ( defined( 'ZINN_CACHE_PRO_WP_REALPATH' ) ) {
				$file_path_ori = $docroot . constant( 'ZINN_CACHE_PRO_WP_REALPATH' ) . $url_parsed['path'];
			} else {
		...
		$file_path_ori = apply_filters( 'zinn_cache_pro_realpath', $file_path_ori );
