...
class Crawler_Map extends Root {
	...
	public function __construct() {
		...
		$this->_conf_map_timeout = defined( 'ZINN_CACHE_PRO_CRAWLER_MAP_TIMEOUT' ) ? constant( 'ZINN_CACHE_PRO_CRAWLER_MAP_TIMEOUT' ) : 180;
	}
	...
	public function empty_map() {
		...
		$msg = __( 'Sitemap cleaned successfully', 'zinn-cache-pro' );
	...
	public function gen( $manual = false ) {
		...
		if ( ! $count ) {
			Admin_Display::error( __( 'No valid sitemap parsed for crawler.', 'zinn-cache-pro' ) );
			return;
		}

		if ( ! wp_doing_cron() && $manual ) {
			$msg = sprintf( __( 'Sitemap created successfully: %d items', 'zinn-cache-pro' ), $count );
			Admin_Display::success( $msg );
		}
	}
	...
	private function _gen() {
		...
		if ( is_array( $this->_urls ) && ! empty( $this->_urls ) ) {
			if ( defined( 'ZINN_CACHE_PRO_CRAWLER_DROP_DOMAIN' ) && constant( 'ZINN_CACHE_PRO_CRAWLER_DROP_DOMAIN' ) ) {
