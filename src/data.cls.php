...
class Data extends Root {
	 ...
	 */
	private $_db_updater = [
		'5.3-a5'    => [ 'zinn_cache_pro_update_5_3' ],
		'7.0-b26'   => [ 'zinn_cache_pro_update_7' ],
		'7.0.1-b1'  => [ 'zinn_cache_pro_update_7_0_1' ],
		'7.7-b28'   => [ 'zinn_cache_pro_update_7_7' ],
	];
	...
	private $_db_site_updater = [
		// '2.0' => [ 'zinn_cache_pro_update_site_2_0' ],
	];
	...
	const TB_IMG_OPTM = 'zinn_cache_pro_img_optm';
	/** Table: image optimization working queue. */
	const TB_IMG_OPTMING = 'zinn_cache_pro_img_optming';
	/** Table: cached avatars. */
	const TB_AVATAR = 'zinn_cache_pro_avatar';
	/** Table: crawler URLs. */
	const TB_CRAWLER = 'zinn_cache_pro_crawler';
	/** Table: crawler blacklist. */
	const TB_CRAWLER_BLACKLIST = 'zinn_cache_pro_crawler_blacklist';
	/** Table: logical URLs. */
	const TB_URL = 'zinn_cache_pro_url';
	/** Table: URL → generated file mapping. */
	const TB_URL_FILE = 'zinn_cache_pro_url_file';
	...
	public function conf_upgrade( $ver ) {
		...
		require_once ZINN_CACHE_PRO_DIR . 'src/data.upgrade.func.php';
		...
		if ( ! defined( 'ZINN_CACHE_PRO_EMPTYCACHE' ) ) {
			define( 'ZINN_CACHE_PRO_EMPTYCACHE', true );
		}
		Purge::purge_all();
	...
	private function _get_upgrade_lock() {
		$is_upgrading = (int) get_option( 'zinn-cache-pro.data.upgrading' );
		if ( ! $is_upgrading ) {
	...
	public function check_upgrading_msg() {
		...
		Admin_Display::info(
			sprintf(
				...
				__( 'The database has been upgrading in the background since %s. This message will disappear once upgrade is complete.', 'zinn-cache-pro' ),
	...
	private function _set_upgrade_lock( $lock ) {
		if ( ! $lock ) {
			update_option( 'zinn-cache-pro.data.upgrading', -1 );
		} else {
			update_option( 'zinn-cache-pro.data.upgrading', time() );
		}
	}
	...
	private function _tb_structure( $tb ) {
		return File::read( ZINN_CACHE_PRO_DIR . 'src/data_structure/' . $tb . '.sql' );
	}
	...
	public function save_url( $request_url, $vary, $file_type, $filecon_md5, $path, $mobile = false, $webp = false ) {
		...
		if ( $file_row ) {
			...
			$expired = time() + ( 86400 * apply_filters( 'zinn_cache_pro_url_file_expired_days', 20 ) );
	...
	private function _load_per_line( $file ) {
		$data = File::read( ZINN_CACHE_PRO_DIR . 'data/' . $file );
