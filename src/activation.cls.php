...
class Activation extends Base {
	...
	public function __construct() {
		self::$data_file = ZINN_CACHE_PRO_CONTENT_DIR . '/' . self::CONF_FILE;
	}
	...
	public static function register_activation() {
		$count = 0;
		! defined( 'ZINN_CACHE_PRO_LOG_TAG' ) && define( 'ZINN_CACHE_PRO_LOG_TAG', 'Activate_' . get_current_blog_id() );
		...
		if ( defined( 'ZINN_CACHE_PRO_REF' ) && 'whm' === ZINN_CACHE_PRO_REF ) {
	 ...
	 * @access public
	 */
	public static function uninstall_zinn_cache_pro_cache() {
		Task::destroy();
		...
		if ( file_exists( ZINN_CACHE_PRO_STATIC_DIR ) ) {
			File::rrmdir( ZINN_CACHE_PRO_STATIC_DIR );
		}
	...
	private static function delete_settings() {
		...
		$wpdb->query($wpdb->prepare("DELETE FROM `$wpdb->options` WHERE option_name LIKE %s", 'zinn-cache-pro.%'));
	}
	...
	private static function get_network_count() {
		...
		foreach ( $sites as $site ) {
			...
			if ( ! empty( $plugins ) && in_array( ZINN_CACHE_PRO_BASENAME, $plugins, true ) ) {
				++$count;
			}
		}
		...
		if ( is_plugin_active_for_network( ZINN_CACHE_PRO_BASENAME ) ) {
			++$count;
		}
		return $count;
	}
	...
	public static function register_deactivation() {
		...
		! defined( 'ZINN_CACHE_PRO_LOG_TAG' ) && define( 'ZINN_CACHE_PRO_LOG_TAG', 'Deactivate_' . get_current_blog_id() );
	 ...
	 *
	 * Removes the .zinn_cache_pro_conf.dat file.
	 *
	 * @since  4.1
	 * @access private
	 */
	 ...
	 *
	 * Updates the .zinn_cache_pro_conf.dat file with relevant settings.
	 *
	 * @since  4.1
	 * @access private
	...
	private function update_conf_data_file( $options ) {
		...
		if ( $old_data !== $data ) {
			defined( 'ZINN_CACHE_PRO_LOG' ) && Debug2::debug( '[Activation] Updating .zinn_cache_pro_conf.dat' );
	...
	public function manage_wp_cache_const( $enable ) {
		...
		if ( apply_filters( 'zinn_cache_pro_wpconfig_readonly', false ) ) {
			throw new \Exception( 'wp-config file is forbidden to modify due to API hook: zinn_cache_pro_wpconfig_readonly' );
		}
		...
		/**
		 * ZINN: refuse to write a wp-config we cannot vouch for (#2709).
		 *
		 * `preg_replace()` returns NULL on a PCRE failure — a backtrack, recursion or JIT
		 * stack limit — without throwing and without warning. The next `preg_replace()` then
		 * receives NULL as its subject and returns an empty string, and
		 * `file_put_contents( $f, '', LOCK_EX )` TRUNCATES THE FILE ON OPEN and returns 0 —
		 * which is not `false`, so `File::save()` reports success and the `true !== $res`
		 * check below never fires.
		 *
		 * The result is a 0-byte wp-config.php owned by the site user, with nothing logged
		 * anywhere, and a site that answers 500 on every PHP path while every static path
		 * still serves 200. Demonstrated by execution on 2026-08-19.
		 *
		 * The pre-existing `if ( ! $content )` check above cannot catch this: it validates the
		 * value we were GIVEN, and what goes wrong is the value we are about to WRITE.
		 * A read guard is not a write guard.
		 */
		if ( ! is_string( $content ) || '' === trim( $content ) ) {
			throw new \Exception(
				'refusing to write wp-config.php: the WP_CACHE rewrite produced empty content '
				. '(preg_last_error=' . preg_last_error() . ' ' . preg_last_error_msg() . '). '
				. 'The file on disk is unchanged.'
			);
		}
		if ( false === strpos( $content, 'wp-settings.php' ) ) {
			// Non-empty is not the same as intact: a config that lost its wp-settings.php
			// require parses fine and boots nothing, fatalling on `undefined function wp()`.
			throw new \Exception(
				'refusing to write wp-config.php: the WP_CACHE rewrite dropped the '
				. 'wp-settings.php require. The file on disk is unchanged.'
			);
		}
	...
	public function auto_update_hook( $update, $item ) {
		if ( ! empty( $item->slug ) && 'zinn-cache-pro' === $item->slug ) {
	...
	public function upgrade() {
		...
		try {
			...
			Admin_Display::error( __( 'Failed to upgrade.', 'zinn-cache-pro' ) );
			return;
		}
		...
		Admin_Display::success( __( 'Upgraded successfully.', 'zinn-cache-pro' ) );
	}
