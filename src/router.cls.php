...
class Router extends Base {
	...
	const NONCE  = 'ZINN_CACHE_PRO_NONCE';
	const ACTION = 'ZINN_CACHE_PRO_CTRL';
	...
	const TYPE = 'zinn_cache_pro_type';
	...
	public static function self_redirect( $action, $type ) {
		...
		$i = !empty($_GET['zinn_cache_pro_i']) ? $_GET['zinn_cache_pro_i'] : 0;
		++$i;

		$link = Utility::build_url($action, $type, false, null, array( 'zinn_cache_pro_i' => $i ));
	...
	public function can_optm() {
		...
		$can_final = apply_filters('zinn_cache_pro_can_optm', $can);
	...
	public static function can_cdn() {
		...
		$can_final = apply_filters('zinn_cache_pro_can_cdn', $can);
	...
	public function is_role_simulation() {
		...
		if (empty($_COOKIE['zinn_cache_pro_hash']) && empty($_COOKIE['zinn_cache_pro_flash_hash'])) {
			return;
		}
		...
		if (!empty($_COOKIE['zinn_cache_pro_flash_hash'])) {
			...
			if ($hash_data && is_array($hash_data) && !empty($hash_data['hash']) && !empty($hash_data['ts']) && !empty($hash_data['uid'])) {
				if (time() - $hash_data['ts'] < 120 && $_COOKIE['zinn_cache_pro_flash_hash'] == $hash_data['hash']) {
		...
		// Hash validation
		if (!empty($_COOKIE['zinn_cache_pro_hash'])) {
			$hash_data = self::get_option(self::ITEM_HASH, array());
			if ($hash_data && is_array($hash_data) && !empty($hash_data['hash']) && !empty($hash_data['ts']) && !empty($hash_data['uid'])) {
				$RUN_DURATION = $this->cls('Crawler')->get_crawler_duration();
				if (time() - $hash_data['ts'] < $RUN_DURATION && $_COOKIE['zinn_cache_pro_hash'] == $hash_data['hash']) {
	...
	public static function get_role( $uid = null ) {
		if (defined('ZINN_CACHE_PRO_WP_ROLE')) {
			return ZINN_CACHE_PRO_WP_ROLE;
		}
		...
		define('ZINN_CACHE_PRO_WP_ROLE', $role);
	...
	public function esi_enabled() {
		if (!isset(self::$_esi_enabled)) {
			self::$_esi_enabled = defined('ZINN_CACHE_PRO_ON') && $this->conf(self::O_ESI);
	...
	public static function get_action() {
		if (!isset(self::$_action)) {
			...
			if (self::$_action) {
				defined('ZINN_CACHE_PRO_LOG') && Debug2::debug('[Router] ZINN_CACHE_PRO_CTRL verified: ' . var_export(self::$_action, true));
			}
		}
		return self::$_action;
	}
	...
	private function verify_action() {
		if (empty($_REQUEST[self::ACTION])) {
			Debug2::debug2('[Router] ZINN_CACHE_PRO_CTRL bypassed empty');
			return;
		}
		...
		if (!$this->verify_nonce($action)) {
			...
			if (!$this->is_admin_ip()) {
				Debug2::debug('[Router] ZINN_CACHE_PRO_CTRL query string - did not match admin IP: ' . $action);
				return;
			}
			...
			if (
				...
				Debug2::debug('[Router] ZINN_CACHE_PRO_CTRL query string - did not match admin IP Actions: ' . $action);
				return;
			}

			if (apply_filters('zinn_cache_pro_qs_forbidden', false)) {
				Debug2::debug('[Router] ZINN_CACHE_PRO_CTRL forbidden by hook zinn_cache_pro_qs_forbidden');
				return;
			}
		...
		Debug2::debug('[Router] ZINN_CACHE_PRO_CTRL: ' . $action);
		...
		switch ($action) {
			case self::ACTION_TMP_DISABLE: // Disable LSC for 24H
				Debug2::tmp_disable();
				Admin::redirect("?page=zinn-cache-pro-toolbox#settings-debug");
				return;
            ...
            if (defined('ZINN_CACHE_PRO_ON') && ($_can_network_option || $_can_option || self::is_ajax())) {
            ...
            if ((defined('ZINN_CACHE_PRO_ON') || $_is_network_admin) && ($_can_network_option || (!$_is_multisite && $_can_option))) {
					self::$_action = $action;
				}
				return;
            ...
            if (defined('ZINN_CACHE_PRO_ON') && ($_is_public_action || self::is_ajax())) {
					self::$_action = $action;
				}
				return;
			...
			default:
            Debug2::debug('[Router] ZINN_CACHE_PRO_CTRL match failed: ' . $action);
				return;
		}
	}
	...
	public function serve_static() {
		if (!empty($_SERVER['SCRIPT_URI'])) {
			if (strpos($_SERVER['SCRIPT_URI'], ZINN_CACHE_PRO_STATIC_URL . '/') !== 0) {
				return;
			}
			$path = substr($_SERVER['SCRIPT_URI'], strlen(ZINN_CACHE_PRO_STATIC_URL . '/'));
		} elseif (!empty($_SERVER['REQUEST_URI'])) {
			$static_path = parse_url(ZINN_CACHE_PRO_STATIC_URL, PHP_URL_PATH) . '/';
