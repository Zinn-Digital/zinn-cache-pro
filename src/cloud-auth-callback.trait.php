...
trait Cloud_Auth_Callback {
	...
	private function _load_server_pk( $from_wpapi = false ) {
		// Removed: the QUIC.cloud server public-key fetch. false is upstream's own failure
		// value; _validate_signature() returns false on it, so signed QC callbacks and the
		// QC activation redirect are rejected.
		unset( $from_wpapi );
		self::debugErr( 'Cloud key loading is disabled in this build' );
		return false;
	...
	public function finish_qc_activation( $ref = false ) {
		...
		if ( ! $is_valid ) {
			...
			Admin_Display::error( sprintf( __( 'Failed to validate %s activation data.', 'zinn-cache-pro' ), 'QUIC.cloud' ) );
			return;
		}
		...
		if ( ! in_array( $qc_activated, [ 'anonymous', 'linked', 'cdn' ], true ) ) {
			...
			Admin_Display::error( sprintf( __( 'Failed to parse %s activation status.', 'zinn-cache-pro' ), 'QUIC.cloud' ) );
			return;
		}
		...
		if ( abs( $diff ) > 86400 ) {
			...
			Admin_Display::error( sprintf( __( '%s activation data expired.', 'zinn-cache-pro' ), 'QUIC.cloud' ) );
			return;
		}
	...
	public function update_qc_activation( $qc_activated, $main_domain = false, $quite = false ) {
		...
		$msg = sprintf( __( 'Congratulations, %s successfully set this domain up for the anonymous online services.', 'zinn-cache-pro' ), 'QUIC.cloud' );
		if ( 'linked' === $qc_activated ) {
			$msg = sprintf( __( 'Congratulations, %s successfully set this domain up for the online services.', 'zinn-cache-pro' ), 'QUIC.cloud' );
	...
	private function _reset_qc_reg_content() {
		$msg  = __( 'Site not recognized. QUIC.cloud deactivated automatically. Please reactivate your QUIC.cloud account.', 'zinn-cache-pro' );
		$msg .= Doc::learn_more( admin_url( 'admin.php?page=zinn-cache-pro' ), __( 'Click here to proceed.', 'zinn-cache-pro' ), true, false, true );
