...
class Task extends Root {
	...
	private static $_triggers = [
		Base::O_IMG_OPTM_CRON => [
			'name' => 'zinn_cache_pro_task_imgoptm_pull',
			'hook' => 'ZinnCachePro\Img_Optm::start_async_cron',
		], // always fetch immediately
		Base::O_OPTM_CSS_ASYNC => [
			'name' => 'zinn_cache_pro_task_ccss',
			'hook' => 'ZinnCachePro\CSS::cron_ccss',
		],
		Base::O_OPTM_UCSS => [
			'name' => 'zinn_cache_pro_task_ucss',
			'hook' => 'ZinnCachePro\UCSS::cron',
		],
		Base::O_MEDIA_VPI_CRON => [
			'name' => 'zinn_cache_pro_task_vpi',
			'hook' => 'ZinnCachePro\VPI::cron',
		],
		Base::O_MEDIA_PLACEHOLDER_RESP_ASYNC => [
			'name' => 'zinn_cache_pro_task_lqip',
			'hook' => 'ZinnCachePro\Placeholder::cron',
		],
		Base::O_DISCUSS_AVATAR_CRON => [
			'name' => 'zinn_cache_pro_task_avatar',
			'hook' => 'ZinnCachePro\Avatar::cron',
		],
		Base::O_IMG_OPTM_AUTO => [
			'name' => 'zinn_cache_pro_task_imgoptm_req',
			'hook' => 'ZinnCachePro\Img_Optm::cron_auto_request',
		],
		Base::O_GUEST => [
			'name' => 'zinn_cache_pro_task_guest_sync',
			'hook' => 'ZinnCachePro\Guest::cron',
		], // Daily sync Guest Mode IP/UA lists
		Base::O_CRAWLER => [
			'name' => 'zinn_cache_pro_task_crawler',
			'hook' => 'ZinnCachePro\Crawler::start_async_cron',
	 ...
	 * @var string
	 */
	const FILTER_CRAWLER = 'zinn_cache_pro_crawl_filter';
	 ...
	 * @var string
	 */
	const FILTER = 'zinn_cache_pro_filter';
	 ...
	 * @return void
	 */
	public static function async_zinn_cache_pro_handler() {
		$hash_data = self::get_option( 'async_call-hash', [] );
		if ( ! $hash_data || ! is_array( $hash_data ) || empty( $hash_data['hash'] ) || empty( $hash_data['ts'] ) ) {
			self::debug( 'async_zinn_cache_pro_handler no hash data', $hash_data );
			return;
		}
		...
		if ( 120 < time() - (int) $hash_data['ts'] || '' === $nonce || $nonce !== $hash_data['hash'] ) {
			self::debug( 'async_zinn_cache_pro_handler nonce mismatch' );
			return;
		}
	...
	public function lscache_cron_filter( $schedules ) {
		if ( ! array_key_exists( self::FILTER, $schedules ) ) {
			$schedules[ self::FILTER ] = [
				'interval' => 900,
				'display'  => __( 'Every 15 Minutes', 'zinn-cache-pro' ),
			];
		}
		return $schedules;
	}
	...
	public function lscache_cron_filter_crawler( $schedules ) {
		$crawler_run_interval = defined( 'ZINN_CACHE_PRO_CRAWLER_RUN_INTERVAL' ) ? (int) constant( 'ZINN_CACHE_PRO_CRAWLER_RUN_INTERVAL' ) : 600;

		if ( ! array_key_exists( self::FILTER_CRAWLER, $schedules ) ) {
			$schedules[ self::FILTER_CRAWLER ] = [
				'interval' => $crawler_run_interval,
				'display'  => __( 'LiteSpeed Crawler Cron', 'zinn-cache-pro' ),
			];
		}
		return $schedules;
	}
