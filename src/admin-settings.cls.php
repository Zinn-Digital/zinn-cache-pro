...
class Admin_Settings extends Base {
	...
	public function save( $raw_data ) {
		...
		if ( empty( $raw_data[ self::ENROLL ] ) ) {
			wp_die( esc_html__( 'No fields', 'zinn-cache-pro' ) );
		}
					...
					if ( $data ) {
						foreach ( $data as $k => $v ) {
							if ( user_can( $v, 'edit_posts' ) ) {
								...
								$msg = sprintf(
									esc_html__( 'The user with id %s has editor access, which is not allowed for the role simulator.', 'zinn-cache-pro' ),
		...
		$msg = __( 'Options saved.', 'zinn-cache-pro' );
