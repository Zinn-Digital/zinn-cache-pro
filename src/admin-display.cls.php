...
class Admin_Display extends Base {
	 ...
	 * @var string
	 */
	const COOKIE_QC_HIDE_BANNER = 'zinn_cache_pro_qc_hide_banner';
	...
	protected static $settings_filters = [
		// Crawler - Blocklist.
		'crawler-blocklist' => [
			'filter' => 'zinn_cache_pro_crawler_disable_blocklist',
			'type'   => 'boolean',
		],
		// Crawler - Settings.
		...
		// Cache - ESI.
		self::O_ESI_NONCE => [
			'filter' => 'zinn_cache_pro_esi_nonces',
		],
		// Page Optimization - CSS.
		'optm-ucss_per_pagetype' => [
			'filter' => 'zinn_cache_pro_ucss_per_pagetype',
			'type'   => 'boolean',
		],
		// Page Optimization - Media.
		self::O_MEDIA_ADD_MISSING_SIZES => [
			'filter' => 'zinn_cache_pro_media_ignore_remote_missing_sizes',
			'type'   => 'boolean',
		],
		// Page Optimization - Media Exclude.
		self::O_MEDIA_LAZY_EXC => [
			'filter' => 'zinn_cache_pro_media_lazy_img_excludes',
		],
		// Page Optimization - Tuning (JS).
		self::O_OPTM_JS_DELAY_INC => [
			'filter' => 'zinn_cache_pro_optm_js_delay_inc',
		],
		self::O_OPTM_JS_EXC => [
			'filter' => 'zinn_cache_pro_optimize_js_excludes',
		],
		self::O_OPTM_JS_DEFER_EXC => [
			'filter' => 'zinn_cache_pro_optm_js_defer_exc',
		],
		self::O_OPTM_GM_JS_EXC => [
			'filter' => 'zinn_cache_pro_optm_gm_js_exc',
		],
		self::O_OPTM_EXC => [
			'filter' => 'zinn_cache_pro_optm_uri_exc',
		],
		// Page Optimization - Tuning (CSS).
		self::O_OPTM_CSS_EXC => [
			'filter' => 'zinn_cache_pro_optimize_css_excludes',
		],
		self::O_OPTM_UCSS_EXC => [
			'filter' => 'zinn_cache_pro_ucss_exc',
		],
	];
	...
	public function __construct() {
		$this->_pages = [
			...
			'zinn-cache-pro-cdn'           => [ 'title' => __( 'CDN', 'zinn-cache-pro' ), 'tpl' => 'cdn/entry.tpl.php', 'scope' => 'site' ],
			'zinn-cache-pro-img_optm'      => [ 'title' => __( 'Image Optimization', 'zinn-cache-pro'), 'tpl' => 'img_optm/entry.tpl.php' ],
			'zinn-cache-pro-page_optm'     => [ 'title' => __( 'Page Optimization', 'zinn-cache-pro' ), 'tpl' => 'page_optm/entry.tpl.php', 'scope' => 'site' ],
			'zinn-cache-pro-db_optm'       => [ 'title' => __( 'Database', 'zinn-cache-pro' ), 'tpl' => 'db_optm/entry.tpl.php' ],
			'zinn-cache-pro-crawler'       => [ 'title' => __( 'Crawler', 'zinn-cache-pro' ), 'tpl' => 'crawler/entry.tpl.php', 'scope' => 'site' ],
			'zinn-cache-pro-toolbox'       => [ 'title' => __( 'Toolbox', 'zinn-cache-pro' ), 'tpl' => 'toolbox/entry.tpl.php' ],
		];
		...
		if ( $this->_is_network_admin && ( is_plugin_active_for_network( ZINN_CACHE_PRO_BASENAME ) || defined( 'ZINN_CACHE_PRO_MU_PLUGIN' ) ) ) {
		...
		add_action( "load-$hook", function () {
			add_filter(
				...
				function ( $footer_text ) {
					...
					require_once ZINN_CACHE_PRO_DIR . 'tpl/inc/admin_footer.php';
					return $footer_text;
				},
				1
			);
			...
			add_filter( 'admin_body_class', function ( $classes ) {
				...
				if ( $screen && in_array( $screen->id, [ 'settings_page_zinn-cache-pro-options', 'toplevel_page_litespeed' ], true ) ) {
					$classes .= ' zinn-cache-pro_page_zinn-cache-pro';
				}
				return $classes;
			} );
		} );
	...
	private function render_page( $slug ) {
		$tpl = ZINN_CACHE_PRO_DIR . 'tpl/' . $this->_pages[ $slug ]['tpl'];
	...
	public function register_admin_menu() {
		...
		$hook = add_options_page(
			...
			function () {
				$this->render_page( 'zinn-cache-pro-cache' );
			}
		);
		$this->bind_page( $hook );
	}
	...
	public function enqueue_style() {
		wp_enqueue_style( Core::PLUGIN_NAME, ZINN_CACHE_PRO_PLUGIN_URL . 'assets/css/litespeed.css', [], Core::VER, 'all' );
        wp_enqueue_style( Core::PLUGIN_NAME . '-dark-mode', ZINN_CACHE_PRO_PLUGIN_URL . 'assets/css/litespeed-dark-mode.css', [], Core::VER, 'all' );
	}
	...
	public function enqueue_scripts() {
		wp_register_script( Core::PLUGIN_NAME, ZINN_CACHE_PRO_PLUGIN_URL . 'assets/js/zinn-cache-pro-admin.js', [], Core::VER, true );
		...
		if ( 'admin.php' === $pagenow && $page && ( 0 === strpos( $page, 'zinn-cache-pro-' ) || 'zinn-cache-pro' === $page ) ) {
			if ( in_array( $page, [ 'zinn-cache-pro-crawler', 'zinn-cache-pro-cdn' ], true ) ) {
				// Babel JS type correction
				add_filter( 'script_loader_tag', [ $this, 'babel_type' ], 10, 3 );

				wp_enqueue_script( Core::PLUGIN_NAME . '-lib-react', ZINN_CACHE_PRO_PLUGIN_URL . 'assets/js/react.min.js', [], Core::VER, false );
				wp_enqueue_script( Core::PLUGIN_NAME . '-lib-babel', ZINN_CACHE_PRO_PLUGIN_URL . 'assets/js/babel.min.js', [], Core::VER, false );
			}

			// Crawler Cookie Simulation
			if ( 'zinn-cache-pro-crawler' === $page ) {
				wp_enqueue_script( Core::PLUGIN_NAME . '-crawler', ZINN_CACHE_PRO_PLUGIN_URL . 'assets/js/component.crawler.js', [], Core::VER, false );

				$localize_data['lang']                              = [];
				$localize_data['lang']['cookie_name']               = __( 'Cookie Name', 'zinn-cache-pro' );
				$localize_data['lang']['cookie_value']              = __( 'Cookie Values', 'zinn-cache-pro' );
				$localize_data['lang']['one_per_line']              = Doc::one_per_line( true );
				$localize_data['lang']['remove_cookie_simulation']  = __( 'Remove cookie simulation', 'zinn-cache-pro' );
				$localize_data['lang']['add_cookie_simulation_row'] = __( 'Add new cookie to simulate', 'zinn-cache-pro' );
			...
			if ( 'zinn-cache-pro-cdn' === $page ) {
				...
				wp_enqueue_script( Core::PLUGIN_NAME . '-cdn', ZINN_CACHE_PRO_PLUGIN_URL . 'assets/js/component.cdn.js', [], Core::VER, false );
				$localize_data['lang']                         = [];
				...
				$localize_data['lang']['cdn_mapping_url_desc'] = sprintf( __( 'CDN URL to be used. For example, %s', 'zinn-cache-pro' ), '<code>' . esc_html( $cdn_url ) . '</code>' );
				$localize_data['lang']['one_per_line']         = Doc::one_per_line( true );
				$localize_data['lang']['cdn_mapping_remove']   = __( 'Remove CDN URL', 'zinn-cache-pro' );
				$localize_data['lang']['add_cdn_mapping_row']  = __( 'Add new CDN URL', 'zinn-cache-pro' );
				$localize_data['lang']['on']                   = __( 'ON', 'zinn-cache-pro' );
				$localize_data['lang']['off']                  = __( 'OFF', 'zinn-cache-pro' );
				if ( empty( $localize_data['ids'] ) ) {
					$localize_data['ids'] = [];
				}
		...
		if ( $show_deactivation_modal && 'plugins.php' === $pagenow ) {
			wp_enqueue_script( Core::PLUGIN_NAME . '-iziModal', ZINN_CACHE_PRO_PLUGIN_URL . 'assets/js/iziModal.min.js', [], Core::VER, true );
			wp_enqueue_style( Core::PLUGIN_NAME . '-iziModal', ZINN_CACHE_PRO_PLUGIN_URL . 'assets/css/iziModal.min.css', [], Core::VER, 'all' );
			add_action( 'admin_footer', [ $this, 'add_deactivation_html' ] );
		}

		if ( $localize_data ) {
			wp_localize_script( Core::PLUGIN_NAME, 'zinn_cache_pro_data', $localize_data );
		}
	...
	public function add_deactivation_html() {
		require ZINN_CACHE_PRO_DIR . 'tpl/inc/modal.deactivation.php';
	}
	...
	public static function build_notice( $color, $str, $irremovable = false, $additional_classes = '' ) {
		...
		return '<div class="zinn_cache_pro_icon ' . esc_attr( $cls ) . '"><p>' . wp_kses_post( $str ) . '</p></div>';
	}
	...
	public static function add_notice( $color, $msg, $do_echo = false, $irremovable = false, $additional_classes = '' ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_CLI' ) || wp_doing_cron() ) {
	...
	public function display_messages() {
		if ( ! defined( 'ZINN_CACHE_PRO_CONF_LOADED' ) ) {
			$this->_in_upgrading();
		}
			...
			foreach ( $messages as $k => $msg ) {
				...
				if ( '</div>' === substr( $msg, -6 ) ) {
					...
					$msg  =
						...
						esc_html__( 'Dismiss', 'zinn-cache-pro' ) .
						'</a>' .
						'</p></div>';
				}
				echo wp_kses_post( $msg );
		...
		if ( empty( $_GET['page'] ) || 0 !== strpos( sanitize_text_field( wp_unslash( $_GET['page'] ) ), 'zinn-cache-pro' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			global $pagenow;
	...
	public function show_widget_edit( $widget, $return_val, $instance ) {
		require ZINN_CACHE_PRO_DIR . 'tpl/esi_widget_edit.php';
	}
	...
	public function show_display_installed() {
		require_once ZINN_CACHE_PRO_DIR . 'tpl/inc/show_display_installed.php';
	}
	...
	public static function show_error_cookie() {
		require_once ZINN_CACHE_PRO_DIR . 'tpl/inc/show_error_cookie.php';
	}
	...
	public function cache_disabled_warning() {
		include ZINN_CACHE_PRO_DIR . 'tpl/inc/check_cache_disabled.php';
	}
	...
	private function _in_upgrading() {
		include ZINN_CACHE_PRO_DIR . 'tpl/inc/in_upgrading.php';
	}
	...
	public function form_end() {
		...
		if ( ! defined( 'ZINN_CACHE_PRO_CONF_LOADED' ) ) {
			submit_button( __( 'Save Changes', 'zinn-cache-pro' ), 'secondary litespeed-duplicate-float', 'litespeed-submit', true, [ 'disabled' => 'disabled' ] );

			echo '</div>';
		} else {
			submit_button(
				__( 'Save Changes', 'zinn-cache-pro' ),
	...
	public function build_toggle( $id, $checked = null, $title_on = null, $title_off = null ) {
		...
		if ( null === $title_on ) {
			$title_on  = __( 'ON', 'zinn-cache-pro' );
			$title_off = __( 'OFF', 'zinn-cache-pro' );
		}
		$cls = $checked ? 'primary' : 'default litespeed-toggleoff';
		echo "<div class='litespeed-toggle litespeed-toggle-btn litespeed-toggle-btn-" . esc_attr( $cls ) . "' data-zinn-cache-pro-toggle-on='primary' data-zinn-cache-pro-toggle-off='default' data-zinn_cache_pro_toggle_id='" . esc_attr( $id ) . "' >
	...
	public function build_switch( $id, $title_list = false ) {
		...
		if ( ! $title_list ) {
			$title_list = [ __( 'OFF', 'zinn-cache-pro' ), __( 'ON', 'zinn-cache-pro' ) ];
		}
	...
	protected function _check_overwritten( $id ) {
		...
		if ( is_bool( $default ) ) {
			$val = $val ? __( 'ON', 'zinn-cache-pro' ) : __( 'OFF', 'zinn-cache-pro' );
		} else {
			if ( is_array( $val ) ) {
				$val = implode( "\n", $val );
			}
		...
		if ( 'textarea' !== $type ) {
			...
			if ( null !== $server_val ) {
				// Show $_SERVER value.
				printf( esc_html__( 'This value is overwritten by the %s variable.', 'zinn-cache-pro' ), '$_SERVER' );
				$val = '$_SERVER["' . $server_val[0] . '"] = ' . $server_val[1];
			} elseif ( null !== $deprecated_filter_val ) {
				// Show filter value.
				echo esc_html__( 'This value is overwritten by the filter.', 'zinn-cache-pro' );
			} elseif ( null !== $const_val ) {
				// Show CONSTANT value.
				printf( esc_html__( 'This value is overwritten by the PHP constant %s.', 'zinn-cache-pro' ), '<code>' . esc_html( Base::conf_const( $id ) ) . '</code>' );
			} elseif ( is_multisite() ) {
				// Show multisite overwrite.
				if ( get_current_blog_id() !== BLOG_ID_CURRENT_SITE && $this->conf( self::NETWORK_O_USE_PRIMARY ) ) {
					echo esc_html__( 'This value is overwritten by the primary site setting.', 'zinn-cache-pro' );
				} else {
					echo esc_html__( 'This value is overwritten by the Network setting.', 'zinn-cache-pro' );
				}
			...
			echo ' ' . sprintf( esc_html__( 'Currently set to %s', 'zinn-cache-pro' ), '<code>' . esc_html( $val ) . '</code>' ) . '</div>';
			...
			<div class="litespeed-desc-wrapper">
				<div class="litespeed-desc"><?php echo esc_html__( 'Value from filter applied', 'zinn-cache-pro' ); ?>:</div>
	...
	public function readable_seconds() {
		echo esc_html__( 'seconds', 'zinn-cache-pro' );
		echo ' <span data-zinn-cache-pro-readable=""></span>';
	}
	...
	public function recommended( $id ) {
		...
		if ( ! is_array( $val ) ) {
			printf(
				'%s: <code>%s</code>',
				esc_html__( 'Default value', 'zinn-cache-pro' ),
				esc_html( $val )
			);
			return;
		}
	...
	protected function _validate_syntax( $id ) {
		...
		foreach ( $val as $v ) {
			if ( ! Utility::syntax_checker( $v ) ) {
				echo '<br /><span class="litespeed-warning"> ❌ ' . esc_html__( 'Invalid rewrite rule', 'zinn-cache-pro' ) . ': <code>' . wp_kses_post( $v ) . '</code></span>';
			}
		}
	}
	...
	protected function _validate_htaccess_path( $id ) {
		...
		if ( '/.htaccess' !== substr( $val, -10 ) ) {
			echo '<br /><span class="litespeed-warning"> ❌ ' . sprintf( esc_html__( 'Path must end with %s', 'zinn-cache-pro' ), '<code>/.htaccess</code>' ) . '</span>';
		}
	}
	...
	protected function _validate_ttl( $id, $min = false, $max = false, $allow_zero = false ) {
		...
		if ( $min && $val < $min && ( ! $allow_zero || 0 !== $val ) ) {
			$tip[] = esc_html__( 'Minimum value', 'zinn-cache-pro' ) . ': <code>' . $min . '</code>.';
		}
		if ( $max && $val > $max ) {
			$tip[] = esc_html__( 'Maximum value', 'zinn-cache-pro' ) . ': <code>' . $max . '</code>.';
		}
		...
		if ( $allow_zero ) {
			$range .= esc_html__( 'Zero, or', 'zinn-cache-pro' ) . ' ';
		}

		if ( $min && $max ) {
			$range .= $min . ' - ' . $max;
		} elseif ( $min ) {
			$range .= esc_html__( 'Larger than', 'zinn-cache-pro' ) . ' ' . $min;
		} elseif ( $max ) {
			$range .= esc_html__( 'Smaller than', 'zinn-cache-pro' ) . ' ' . $max;
		}

		echo esc_html__( 'Value range', 'zinn-cache-pro' ) . ': <code>' . esc_html( $range ) . '</code>';
	}
	...
	protected function _validate_ip( $id ) {
		...
		foreach ( $val as $v ) {
			...
			if ( ! \WP_Http::is_ip_address( $v ) ) {
				$tip[] = esc_html__( 'Invalid IP', 'zinn-cache-pro' ) . ': <code>' . esc_html( $v ) . '</code>.';
			}
		}
	...
	protected function _api_env_var( ...$args ) {
		echo '<span class="litespeed-success"> ' .
			esc_html__( 'API', 'zinn-cache-pro' ) . ': ' .
			sprintf(
				/* translators: %s: list of server variables in <code> tags */
				esc_html__( 'Server variable(s) %s available to override this setting.', 'zinn-cache-pro' ),
	...
	protected function _uri_usage_example() {
		echo esc_html__( 'The URLs will be compared to the REQUEST_URI server variable.', 'zinn-cache-pro' );
		/* translators: 1: example URL, 2: pattern example */
		echo ' ' . sprintf( esc_html__( 'For example, for %1$s, %2$s can be used here.', 'zinn-cache-pro' ), '<code>/mypath/mypage?aa=bb</code>', '<code>mypage?aa=</code>' );
		echo '<br /><i>';
		/* translators: %s: caret symbol */
		printf( esc_html__( 'To match the beginning, add %s to the beginning of the item.', 'zinn-cache-pro' ), '<code>^</code>' );
		/* translators: %s: dollar symbol */
		echo ' ' . sprintf( esc_html__( 'To do an exact match, add %s to the end of the URL.', 'zinn-cache-pro' ), '<code>$</code>' );
		echo ' ' . esc_html__( 'One per line.', 'zinn-cache-pro' );
		echo '</i>';
	}
	...
	public static function print_plural( $num, $kind = 'group' ) {
		if ( $num > 1 ) {
			switch ( $kind ) {
				case 'group':
					return sprintf( esc_html__( '%s groups', 'zinn-cache-pro' ), $num );

				case 'image':
					return sprintf( esc_html__( '%s images', 'zinn-cache-pro' ), $num );
		...
		switch ( $kind ) {
			case 'group':
				return sprintf( esc_html__( '%s group', 'zinn-cache-pro' ), $num );

			case 'image':
				return sprintf( esc_html__( '%s image', 'zinn-cache-pro' ), $num );
