...
class Control extends Root {
	...
	public function init() {
		 ...
		 * @since  1.6.2
		 */
		add_filter( 'zinn_cache_pro_vary', [ $this, 'vary_add_role_exclude' ] );
	...
	public function finalize() {
		...
		if ( file_exists( ZINN_CACHE_PRO_DIR . 'src/metabox.cls.php' ) && $this->cls( 'Metabox' )->setting( 'zinn_cache_pro_no_cache' ) ) {
		...
		do_action( 'zinn_cache_pro_control_finalize', defined( 'LSCACHE_IS_ESI' ) ? LSCACHE_IS_ESI : false ); // Pass ESI block id.
		...
		$is_mobile_conf = apply_filters( 'zinn_cache_pro_is_mobile', false );
	...
	/**
	 * Check if is mobile for filter `zinn_cache_pro_is_mobile` in API.
	 *
	 * @since 3.0
	 * @access public
	 * @return bool
	...
	private function _get_req_method() {
		if ( isset( $_SERVER['HTTP_X_HTTP_METHOD_OVERRIDE'] ) ) {
			...
			if ( ! defined( 'ZINN_CACHE_PRO_X_HTTP_METHOD_OVERRIDE' ) ) {
				define( 'ZINN_CACHE_PRO_X_HTTP_METHOD_OVERRIDE', true );
			}
			return $override;
		}
	...
	private function _setting_cacheable() {
		...
		if ( defined( 'ZINN_CACHE_PRO_X_HTTP_METHOD_OVERRIDE' ) && ZINN_CACHE_PRO_X_HTTP_METHOD_OVERRIDE && 'HEAD' === $method ) {
