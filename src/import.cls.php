...
class Import extends Base {
	...
	public function import( $file = false ) {
		if (!$file) {
			if (empty($_FILES['ls_file']['name']) || substr($_FILES['ls_file']['name'], -5) != '.data' || empty($_FILES['ls_file']['tmp_name'])) {
				...
				$msg = __('Import failed due to file error.', 'zinn-cache-pro');
		...
		if (!$file) {
			...
			$msg = sprintf(__('Imported setting file %s successfully.', 'zinn-cache-pro'), $_FILES['ls_file']['name']);
	...
	public function reset() {
		...
		$msg = __('Reset successfully.', 'zinn-cache-pro');
		Admin_Display::success($msg);
	}
