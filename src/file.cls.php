...
class File {
	...
	public static function save( $filename, $data, $mkdir = false, $append = false, $silence = true ) {
		if (is_null($filename)) {
			return $silence ? false : __('Filename is empty!', 'zinn-cache-pro');
		}
		...
		if (!file_exists($folder)) {
			if (!$mkdir) {
				return $silence ? false : sprintf(__('Folder does not exist: %s', 'zinn-cache-pro'), $folder);
			}
			...
			try {
				...
				if (!file_exists(ZINN_CACHE_PRO_STATIC_DIR . '/robots.txt')) {
					file_put_contents(ZINN_CACHE_PRO_STATIC_DIR . '/robots.txt', "User-agent: *\nDisallow: /\n");
				}
			} catch (\ErrorException $ex) {
				return $silence ? false : sprintf(__('Can not create folder: %1$s. Error: %2$s', 'zinn-cache-pro'), $folder, $ex->getMessage());
			}
		...
		if (!file_exists($filename)) {
			if (!is_writable($folder)) {
				return $silence ? false : sprintf(__('Folder is not writable: %s.', 'zinn-cache-pro'), $folder);
			}
			set_error_handler('zinn_cache_pro_exception_handler');
			try {
				touch($filename);
			} catch (\ErrorException $ex) {
				return $silence ? false : sprintf(__('File %s is not writable.', 'zinn-cache-pro'), $filename);
			}
			restore_error_handler();
		...
		if ($ret === false) {
			return $silence ? false : sprintf(__('Failed to write to %s.', 'zinn-cache-pro'), $filename);
		}
