...
class Admin extends Root {
	...
	public function __construct() {
		// Define ZINN_CACHE_PRO_MU_PLUGIN if in mu-plugins.
		if ( defined( 'WPMU_PLUGIN_DIR' ) && dirname( ZINN_CACHE_PRO_DIR ) === WPMU_PLUGIN_DIR && ! defined( 'ZINN_CACHE_PRO_MU_PLUGIN' ) ) {
			define( 'ZINN_CACHE_PRO_MU_PLUGIN', true );
		}
		...
		add_filter(
			'plugin_action_links_' . ZINN_CACHE_PRO_BASENAME,
	...
	public function admin_init() {
		...
		do_action( 'zinn_cache_pro_after_admin_init' );
	...
	/**
	 * After a ZINN_CACHE_PRO_CTRL action, redirect back to same page
	...
	public static function redirect( $url = false ) {
		...
		if ( ! empty( $_GET['_zinn_cache_pro_ori'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$ref = wp_get_referer();
		...
		if ( ! $url ) {
			...
			if ( ! empty( $_GET ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				foreach ( $_GET as $k => $v ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					if ( in_array( $k, [ Router::ACTION, Router::NONCE, Router::TYPE, 'zinn_cache_pro_i', 'zinn_cache_pro_tb' ], true ) ) {
						continue;
					}
