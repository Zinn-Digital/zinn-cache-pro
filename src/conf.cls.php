...
class Conf extends Base {
	...
	public function init() {
		...
		if ( $this->conf( self::O_CDN_QUIC ) ) {
			if ( ! defined( 'ZINN_CACHE_PRO_ALLOWED' ) ) {
				define( 'ZINN_CACHE_PRO_ALLOWED', true );
			}
		}

		add_action( 'zinn_cache_pro_conf_append', [ $this, 'option_append' ], 10, 2 );
		add_action( 'zinn_cache_pro_conf_force', [ $this, 'force_option' ], 10, 2 );
	...
	private function _conf_db_init() {
		...
		$ver_check_tag = 'new';
		if ( $ver ) {
			if ( ! defined( 'ZINN_CACHE_PRO_CUR_V' ) ) {
				define( 'ZINN_CACHE_PRO_CUR_V', $ver );
			}
		...
		if ( ! $ver || Core::VER !== $ver ) {
			...
			if ( ! $ver ) {
				...
				$ver_check_tag .= ' activate' . ( defined( 'ZINN_CACHE_PRO_REF' ) ? '_' . constant( 'ZINN_CACHE_PRO_REF' ) : '' );
			}
		...
		if ( ! defined( 'ZINN_CACHE_PRO_CONF_LOADED' ) ) {
			define( 'ZINN_CACHE_PRO_CONF_LOADED', true );
		}
	...
	public function load_options( $blog_id = null, $dry_run = false ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_CONF' ) && ZINN_CACHE_PRO_CONF ) {
			foreach ( self::$_default_options as $k => $v ) {
	...
	private function _conf_site_db_init() {
		 ...
		 */
		if ( ! $ver || Core::VER !== $ver ) {
			if ( ! is_admin() && ! defined( 'ZINN_CACHE_PRO_CLI' ) ) {
	...
	private function _define_cache_on() {
		...
		if ( defined( 'ZINN_CACHE_PRO_ALLOWED' ) && ! defined( 'ZINN_CACHE_PRO_ON' ) ) {
			define( 'ZINN_CACHE_PRO_ON', true );
		}
	}
	...
	public function update_confs( $the_matrix = [] ) {
		...
		do_action( 'zinn_cache_pro_update_confs', $the_matrix );
	...
	public function update( $id, $val ) {
		...
		if ( self::O_SERVER_IP === $id ) {
			if ( $val && ! Utility::valid_ipv4( $val ) ) {
				$msg = sprintf( __( 'Saving option failed. IPv4 only for %s.', 'zinn-cache-pro' ), Lang::title( Base::O_SERVER_IP ) );
	...
	private function _set_conf() {
		...
		$msg = __( 'Changed setting successfully.', 'zinn-cache-pro' );
