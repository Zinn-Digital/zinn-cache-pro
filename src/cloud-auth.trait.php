...
trait Cloud_Auth {
	...
	public function init_qc() {
		...
		if ( false === $echobox ) {
			...
			$msg = __( "QUIC.cloud's access to your WP REST API seems to be blocked.", 'zinn-cache-pro' );
		...
		if ( empty( $echobox['wpapi_ts'] ) || empty( $echobox['wpapi_signature_b64'] ) ) {
			Admin_Display::error( __( 'Failed to get echo data from WPAPI', 'zinn-cache-pro' ) );
			wp_safe_redirect( $ref );
			exit;
		}
	...
	private function _get_ref_url( $ref = false ) {
		$link = 'admin.php?page=zinn-cache-pro';
		if ( 'cdn' === $ref ) {
			$link = 'admin.php?page=zinn-cache-pro-cdn';
		}
		if ( 'online' === $ref ) {
			$link = 'admin.php?page=zinn-cache-pro-general';
		}
	...
	public function init_qc_cli() {
		...
		if ( ! $server_ip ) {
			...
			$msg = sprintf(
				__( 'You need to set the %1$s first. Please use the command %2$s to set.', 'zinn-cache-pro' ),
				'`' . __( 'Server IP', 'zinn-cache-pro' ) . '`',
				'`wp zinn-cache-pro-option set server_ip __your_ip_value__`'
			);
			Admin_Display::error( $msg );
			return;
		}
	...
	public function init_qc_cdn_cli( $method, $cert = false, $key = false, $cf_token = false ) {
		if ( ! $this->activated() ) {
			Admin_Display::error( __( 'You need to activate QC first.', 'zinn-cache-pro' ) );
			return;
		}
		...
		if ( $cert ) {
			if ( ! file_exists( $cert ) || ! file_exists( $key ) ) {
				Admin_Display::error( __( 'Cert or key file does not exist.', 'zinn-cache-pro' ) );
				return;
			}
		}
	...
	public function reset_qc() {
		...
		Admin_Display::success( sprintf( __( 'Reset %s activation successfully.', 'zinn-cache-pro' ), 'QUIC.cloud' ) );
