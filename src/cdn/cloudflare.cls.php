...
namespace ZinnCachePro\CDN;

use ZinnCachePro\Base;
use ZinnCachePro\Debug2;
use ZinnCachePro\Router;
use ZinnCachePro\Admin;
use ZinnCachePro\Admin_Display;
...
class Cloudflare extends Base {
	...
	private function set_devmode( $type ) {
		...
		if ($res) {
			$msg = sprintf(__('Notified Cloudflare to set development mode to %s successfully.', 'zinn-cache-pro'), strtoupper($new_val));
	...
	private function purge_all_private() {
		...
		if (!$cf_on) {
			$msg = __('Cloudflare API is set to off.', 'zinn-cache-pro');
		...
		if ($res) {
			$msg = __('Notified Cloudflare to purge all successfully.', 'zinn-cache-pro');
			Admin_Display::success($msg);
		}
	}
	...
	private function zone() {
		...
		if (!$zone) {
			$msg = __('No available Cloudflare zone', 'zinn-cache-pro');
			Admin_Display::error($msg);
			return false;
		}
	...
	private function cloudflare_call( $url, $method = 'GET', $data = false, $show_msg = true ) {
		...
		if (is_wp_error($resp)) {
			...
			if ($show_msg) {
				$msg = __('Failed to communicate with Cloudflare', 'zinn-cache-pro');
				Admin_Display::error($msg);
			}
			return false;
		}
		...
		if ($json && $json['success'] && $json['result']) {
			...
			if ($show_msg) {
				$msg = __('Communicated with Cloudflare successfully.', 'zinn-cache-pro');
				Admin_Display::success($msg);
			}
