...
trait Img_Optm_Pull {
	...
	public static function start_async() {
		...
		$msg = __( 'Started async image optimization request', 'zinn-cache-pro' );
	...
	public static function async_handler( $force = false ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_IMG_OPTM_PULL_CRON' ) && ! constant( 'ZINN_CACHE_PRO_IMG_OPTM_PULL_CRON' ) ) {
			self::debug( 'Cron disabled [define] ZINN_CACHE_PRO_IMG_OPTM_PULL_CRON' );
			return;
		}
	...
	private function _calc_pull_threads() {
		...
		if ( defined( 'ZINN_CACHE_PRO_IMG_OPTM_PULL_THREADS' ) ) {
			return constant( 'ZINN_CACHE_PRO_IMG_OPTM_PULL_THREADS' );
		}
	...
	public function pull( $manual = false ) {
		// Removed: image optimisation was a QUIC.cloud-only service, and this loop pulled
		// the optimised files straight from *.quic.cloud nodes (with sslverify off).
		// Nothing can reach the notified state any more, so there is nothing to pull.
		unset( $manual );
		self::debug( 'Image pull is disabled in this build' );
		return;
		...
		if ( $this->cron_running() ) {
			...
			$msg = __( 'Pull Cron is running', 'zinn-cache-pro' );
			...
			while ( $img_rows = $wpdb->get_results( $_q ) ) {
				...
				$complete_action = function ( $response, $req_count ) use ( $imgs_by_req, $rm_ori_bkup, &$total_pulled_ori, &$total_pulled_webp, &$total_pulled_avif, &$server_list ) {
					...
					if ( empty( $response->success ) || empty( $response->status_code ) || 200 !== $response->status_code ) {
						...
						$msg = __( 'Some optimized image file(s) has expired and was cleared.', 'zinn-cache-pro' );
					...
					if ( 'webp' === $row_type ) {
						...
						if ( ! file_exists( $local_file . '.webp' ) || ! filesize( $local_file . '.webp' ) || md5_file( $local_file . '.webp' ) !== $server_info['webp_md5'] ) {
							...
							$msg = __( 'Pulled WebP image md5 does not match the notified WebP image md5.', 'zinn-cache-pro' );
							Admin_Display::error( $msg );
							return;
						}
						 ...
						 */
						do_action( 'zinn_cache_pro_img_pull_webp', $row_img, $local_file . '.webp' );
						...
						if ( ! file_exists( $local_file . '.avif' ) || ! filesize( $local_file . '.avif' ) || md5_file( $local_file . '.avif' ) !== $server_info['avif_md5'] ) {
							...
							$msg = __( 'Pulled AVIF image md5 does not match the notified AVIF image md5.', 'zinn-cache-pro' );
							Admin_Display::error( $msg );
							return;
						}
						 ...
						 * @since 7.0
						 */
						do_action( 'zinn_cache_pro_img_pull_avif', $row_img, $local_file . '.avif' );
						...
						if ( ! file_exists( $local_file . '.tmp' ) || ! filesize( $local_file . '.tmp' ) || md5_file( $local_file . '.tmp' ) !== $server_info['ori_md5'] ) {
							...
							$msg = __( 'One or more pulled images does not match with the notified image md5', 'zinn-cache-pro' );
							Admin_Display::error( $msg );
							return;
						}
						 ...
						 */
						do_action( 'zinn_cache_pro_img_pull_ori', $row_img, $local_file );
				...
				$force_wp_remote_get = defined( 'ZINN_CACHE_PRO_FORCE_WP_REMOTE_GET' ) && constant( 'ZINN_CACHE_PRO_FORCE_WP_REMOTE_GET' );
		...
		// $msg = sprintf(__('Pulled %d image(s)', 'zinn-cache-pro'), $total_pulled_ori + $total_pulled_webp);
		// Admin_Display::success($msg);
