...
class CSS extends Base {
	...
	public function __construct() {
		...
		add_filter( 'zinn_cache_pro_ccss_whitelist', [ $this->cls( 'Data' ), 'load_ccss_whitelist' ] );
	}
	...
	private function _ccss() {
		...
		$this->_queue[ $queue_k ] = [
			'url'        => apply_filters( 'zinn_cache_pro_ccss_url', $request_url ),
	...
	public function prepare_html( $request_url, $user_agent, $uid = false ) {
		$html = $this->cls( 'Crawler' )->self_curl( add_query_arg( 'ZINN_CACHE_PRO_CTRL', 'before_optm', $request_url ), $user_agent, $uid );
	...
	private function _filter_whitelist() {
		$whitelist = [];
		$list      = apply_filters( 'zinn_cache_pro_ccss_whitelist', $this->conf( self::O_OPTM_CCSS_SELECTOR_WHITELIST ) );
		foreach ( $list as $v ) {
			if ( substr( $v, 0, 2 ) === '//' ) {
				continue;
			}
