...
abstract class Root {
	...
	const CONF_FILE = '.zinn_cache_pro_conf.dat';
	// Instance set
	...
	protected function _separate_mobile() {
		return (wp_is_mobile() || apply_filters('zinn_cache_pro_is_mobile', false)) && $this->conf(Base::O_CACHE_MOBILE);
	}
	...
	public function has_cache_folder( $type ) {
		...
		if (file_exists(ZINN_CACHE_PRO_STATIC_DIR . '/' . $type . '/' . $subsite_id)) {
			return true;
		}
		return false;
	}
	...
	protected function _maybe_mk_cache_folder( $type ) {
		if (!$this->has_cache_folder($type)) {
			$subsite_id = is_multisite() && !is_network_admin() ? get_current_blog_id() : '';
			$path       = ZINN_CACHE_PRO_STATIC_DIR . '/' . $type . '/' . $subsite_id;
	...
	public function rm_cache_folder( $type ) {
		...
		File::rrmdir(ZINN_CACHE_PRO_STATIC_DIR . '/' . $type . '/' . $subsite_id);
	...
	public function load_queue( $type ) {
		...
		$static_path     = ZINN_CACHE_PRO_STATIC_DIR . $filepath_prefix . '.zinn_cache_pro_conf.dat';
	...
	public function clear_q( $type, $silent = false ) {
		...
		if (!$silent) {
			$msg = __('All QUIC.cloud service queues have been cleared.', 'zinn-cache-pro');
	...
	public function filter_overwritten( $id ) {
		...
		$filter_name = 'zinn_cache_pro_conf_load_option_' . $id;
	...
	public function conf( $id, $ori = false ) {
		if (isset(self::$_options[$id])) {
			if (!$ori) {
				...
				if ($val !== null) {
					defined('ZINN_CACHE_PRO_LOG') && Debug2::debug('[Conf] 🏛️ const option ' . $id . '=' . var_export($val, true));
					return $val;
				}
		...
		defined('ZINN_CACHE_PRO_LOG') && Debug2::debug('[Conf] Invalid option ID ' . $id);
