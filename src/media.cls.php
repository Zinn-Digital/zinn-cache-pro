...
class Media extends Root {
	...
	public function __construct() {
		...
		if ( $this->conf( Base::O_IMG_OPTM_WEBP ) ) {
			...
			$this->_format = apply_filters( 'zinn_cache_pro_next_gen_format', $this->_format );
		}
	}
	...
	public function init() {
		...
		if ( $this->conf( Base::O_MEDIA_LAZY ) && ! $this->cls( 'Metabox' )->setting( 'zinn_cache_pro_no_image_lazy' ) ) {
		...
		add_filter( 'zinn_cache_pro_buffer_finalize', [ $this, 'finalize' ], 4 );
		add_filter( 'zinn_cache_pro_optm_html_head', [ $this, 'finalize_head' ] );
	}
	...
	private function _batch_rescale_ori() {
		...
		$offset = ! empty( $_GET['zinn_cache_pro_i'] ) ? absint( wp_unslash( $_GET['zinn_cache_pro_i'] ) ) : 0;
		$limit  = 500;
		$count  = 0;
		...
		Admin_Display::success( sprintf( __( 'Batch rescale completed.', 'zinn-cache-pro' ) ) );
	}
	...
	public function after_admin_init() {
		...
		add_action( 'zinn_cache_pro_media_row', [ $this, 'media_row_con' ] );
	}
	...
	public function info( $short_file_path, $post_id ) {
		 ...
		 */
		$info = apply_filters( 'zinn_cache_pro_media_info', [], $short_file_path, $post_id );
	...
	public function del( $short_file_path, $post_id ) {
		...
		do_action( 'zinn_cache_pro_media_del', $short_file_path, $post_id );
	}
	...
	public function rename( $short_file_path, $short_file_path_new, $post_id ) {
		...
		do_action( 'zinn_cache_pro_media_rename', $short_file_path, $short_file_path_new, $post_id );
	}
	...
	public function media_row_title( $posts_columns ) {
		$posts_columns['imgoptm'] = esc_html__( 'LiteSpeed Optimization', 'zinn-cache-pro' );
	...
	public function media_row_actions( $column_name, $post_id ) {
		...
		do_action( 'zinn_cache_pro_media_row', $post_id );
	}
	...
	public function media_row_con( $post_id ) {
		...
		if ( $size_meta && ! empty( $size_meta['ori_saved'] ) ) {
			...
			if ( $this->info( $bk_file, $post_id ) ) {
				$curr_status = esc_html__( '(optm)', 'zinn-cache-pro' );
				$desc        = esc_attr__( 'Currently using optimized version of file.', 'zinn-cache-pro' ) . '&#10;' . esc_attr__( 'Click to switch to original (unoptimized) version.', 'zinn-cache-pro' );
			} elseif ( $this->info( $bk_optm_file, $post_id ) ) {
				$cls        .= ' litespeed-warning';
				$curr_status = esc_html__( '(non-optm)', 'zinn-cache-pro' );
				$desc        = esc_attr__( 'Currently using original (unoptimized) version of file.', 'zinn-cache-pro' ) . '&#10;' . esc_attr__( 'Click to switch to optimized version.', 'zinn-cache-pro' );
			}
			...
			echo wp_kses(
				GUI::pie_tiny(
					...
					24,
					sprintf(
						esc_html__( 'Original file reduced by %1$s (%2$s)', 'zinn-cache-pro' ),
						$percent . '%',
			...
			printf(
				esc_html__( 'Orig saved %s', 'zinn-cache-pro' ),
				(int) $percent . '%'
			);
			...
			if ( $desc ) {
				...
				printf(
					...
					esc_attr__( 'Using optimized version of file. ', 'zinn-cache-pro' ) . '&#10;' . esc_attr__( 'No backup of original file exists.', 'zinn-cache-pro' ),
					esc_html__( '(optm)', 'zinn-cache-pro' )
				);
			}
		} elseif ( $size_meta && 0 === (int) $size_meta['ori_saved'] ) {
			echo wp_kses( GUI::pie_tiny( 0, 24, esc_html__( 'Congratulation! Your file was already optimized', 'zinn-cache-pro' ), 'left' ), GUI::allowed_svg_tags() );
			printf(
				esc_html__( 'Orig %s', 'zinn-cache-pro' ),
				'<span class="litespeed-desc">' . esc_html__( '(no savings)', 'zinn-cache-pro' ) . '</span>'
			);
		} else {
			echo esc_html__( 'Orig', 'zinn-cache-pro' ) . '<span class="litespeed-left10">—</span>';
		}
		echo '</p>';
		...
		if ( $size_meta && $this->webp_support( true ) && ! empty( $size_meta[ $this->_sys_format . '_saved' ] ) ) {
			...
			if ( $this->info( $short_path . '.' . $this->_sys_format, $post_id ) ) {
				$curr_status = esc_html__( '(optm)', 'zinn-cache-pro' );
				$desc        = $is_avif
					? esc_attr__( 'Currently using optimized version of AVIF file.', 'zinn-cache-pro' )
					: esc_attr__( 'Currently using optimized version of WebP file.', 'zinn-cache-pro' );
				$desc       .= '&#10;' . esc_attr__( 'Click to switch to original (unoptimized) version.', 'zinn-cache-pro' );
			} elseif ( $this->info( $short_path . '.optm.' . $this->_sys_format, $post_id ) ) {
				$cls        .= ' litespeed-warning';
				$curr_status = esc_html__( '(non-optm)', 'zinn-cache-pro' );
				$desc        = $is_avif
					? esc_attr__( 'Currently using original (unoptimized) version of AVIF file.', 'zinn-cache-pro' )
					: esc_attr__( 'Currently using original (unoptimized) version of WebP file.', 'zinn-cache-pro' );
				$desc       .= '&#10;' . esc_attr__( 'Click to switch to optimized version.', 'zinn-cache-pro' );
			}
			...
			echo wp_kses(
				GUI::pie_tiny(
					...
					24,
					sprintf(
						$is_avif ? esc_html__( 'AVIF file reduced by %1$s (%2$s)', 'zinn-cache-pro' ) : esc_html__( 'WebP file reduced by %1$s (%2$s)', 'zinn-cache-pro' ),
						$percent . '%',
			...
			printf(
				$is_avif ? esc_html__( 'AVIF saved %s', 'zinn-cache-pro' ) : esc_html__( 'WebP saved %s', 'zinn-cache-pro' ),
			...
			if ( $desc ) {
				...
				printf(
					...
					esc_attr__( 'Using optimized version of file. ', 'zinn-cache-pro' ),
					$is_avif ? esc_attr__( 'No backup of unoptimized AVIF file exists.', 'zinn-cache-pro' ) : esc_attr__( 'No backup of unoptimized WebP file exists.', 'zinn-cache-pro' ),
					esc_html__( '(optm)', 'zinn-cache-pro' )
				);
			}
		} else {
		...
		if ( $size_meta ) {
			printf(
				...
				esc_html__( 'Restore from backup', 'zinn-cache-pro' )
			);
			echo '</div>';
		}
	}
	...
	public function finalize( $content ) {
		if ( defined( 'ZINN_CACHE_PRO_NO_LAZY' ) ) {
	...
	private function _finalize() {
		...
		$cfg_lazy          = ( defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) || $this->conf( Base::O_MEDIA_LAZY ) ) && ! $this->cls( 'Metabox' )->setting( 'zinn_cache_pro_no_image_lazy' );
		$cfg_iframe_lazy   = defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) || $this->conf( Base::O_MEDIA_IFRAME_LAZY );
		$cfg_js_delay      = defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) || 2 === $this->conf( Base::O_OPTM_JS_DEFER );
		$cfg_trim_noscript = defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) || $this->conf( Base::O_OPTM_NOSCRIPT_RM );
		$cfg_vpi           = defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) || $this->conf( Base::O_MEDIA_VPI );
		...
		if ( $cfg_lazy ) {
			if ( $cfg_vpi ) {
				add_filter( 'zinn_cache_pro_media_lazy_img_excludes', [ $this->cls( 'Metabox' ), 'lazy_img_excludes' ] );
			}
		...
		if ( $cfg_iframe_lazy ) {
			...
			foreach ( $html_list as $k => $v ) {
				...
				if ( $cfg_js_delay ) {
					$v = str_replace( ' src=', ' data-zinn-cache-pro-src=', $v );
				} else {
		...
		if ( $cfg_lazy || $cfg_iframe_lazy ) {
			$lazy_lib = '<script data-no-optimize="1">window.lazyLoadOptions=Object.assign({},{threshold:' . apply_filters( 'zinn_cache_pro_lazyload_threshold', 300 ) . '},window.lazyLoadOptions||{});' . File::read( ZINN_CACHE_PRO_DIR . self::LIB_FILE_IMG_LAZYLOAD ) . '</script>';
			if ( $cfg_js_delay ) {
				// Load JS delay lib.
				if ( ! defined( 'ZINN_CACHE_PRO_JS_DELAY_LIB_LOADED' ) ) {
					define( 'ZINN_CACHE_PRO_JS_DELAY_LIB_LOADED', true );
					$lazy_lib .= '<script data-no-optimize="1">' . File::read( ZINN_CACHE_PRO_DIR . Optimize::LIB_FILE_JS_DELAY ) . '</script>';
				}
			}
	...
	private function _parse_img() {
		 ...
		 */
		$excludes = apply_filters( 'zinn_cache_pro_media_lazy_img_excludes', $this->conf( Base::O_MEDIA_LAZY_EXC ) );

		$cls_excludes   = apply_filters( 'zinn_cache_pro_media_lazy_img_cls_excludes', $this->conf( Base::O_MEDIA_LAZY_CLS_EXC ) );
		 ...
		 * @since 3.0
		 */
		$parent_cls_exc = apply_filters( 'zinn_cache_pro_media_lazy_img_parent_cls_excludes', $this->conf( Base::O_MEDIA_LAZY_PARENT_CLS_EXC ) );
		if ( $parent_cls_exc ) {
		...
		$add_missing_sizes = ( defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) || $this->conf( Base::O_MEDIA_ADD_MISSING_SIZES ) )
			&& apply_filters( 'zinn_cache_pro_media_add_missing_sizes', true );
	...
	private function _detect_dimensions( $src ) {
		...
		if ( $pathinfo ) {
			$src = $pathinfo[0];
		} elseif ( apply_filters( 'zinn_cache_pro_media_ignore_remote_missing_sizes', false ) ) {
			return false;
		}
	...
	private function _parse_iframe() {
		$cls_excludes   = apply_filters( 'zinn_cache_pro_media_iframe_lazy_cls_excludes', $this->conf( Base::O_MEDIA_IFRAME_LAZY_CLS_EXC ) );
		$cls_excludes[] = 'skip-lazy'; // https://core.trac.wordpress.org/ticket/44427
		 ...
		 * @since 3.0
		 */
		$parent_cls_exc = apply_filters( 'zinn_cache_pro_media_iframe_lazy_parent_cls_excludes', $this->conf( Base::O_MEDIA_IFRAME_LAZY_PARENT_CLS_EXC ) );
		if ( $parent_cls_exc ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( apply_filters( 'zinn_cache_pro_iframe_lazyload_exc', false, $attrs['src'] ) ) {
	...
	private function _replace_buffer_img_webp( $content ) {
		...
		if ( ( defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) || $this->conf( Base::O_IMG_OPTM_WEBP_REPLACE_SRCSET ) ) && $this->webp_support() ) {
			$content = Utility::srcset_replace( $content, [ $this, 'replace_webp' ] );
		}

		// Replace background-image.
		if ( ( defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) || $this->conf( Base::O_IMG_OPTM_WEBP ) ) && $this->webp_support() ) {
	...
	public function replace_webp( $url ) {
		 ...
		 */
		$ori_check = apply_filters( 'zinn_cache_pro_media_check_ori', Utility::is_internal_file( $url ), $url );
		if ( $ori_check ) {
			// check if has webp/avif file.
			$has_next = apply_filters( 'zinn_cache_pro_media_check_webp', Utility::is_internal_file( $url, $this->_sys_format ), $url );
			if ( $has_next ) {
