...
class Tool extends Root {
	...
	public function check_ip() {
		...
		// Removed: an undisclosed third-party lookup (https://cyberpanel.sh/?ip, sent with
		// a spoofed `curl/8.7.1` User-Agent). Report the server's own address instead.
		$ip = isset( $_SERVER['SERVER_ADDR'] ) ? trim( sanitize_text_field( wp_unslash( $_SERVER['SERVER_ADDR'] ) ) ) : '';
		...
		return esc_html__( 'Failed to detect IP', 'zinn-cache-pro' );
	}
	...
	public function is_editor() {
		...
		return apply_filters( 'zinn_cache_pro_is_editor', $res );
	}
}
