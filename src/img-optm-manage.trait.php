...
trait Img_Optm_Manage {
	...
	public function clean() {
		...
		$msg = __( 'Cleaned up unfinished data successfully.', 'zinn-cache-pro' );
	...
	private function _reset_counter() {
		...
		$msg = __( 'Reset image optimization counter successfully.', 'zinn-cache-pro' );
		Admin_Display::success( $msg );
	}
	...
	private function _destroy() {
		...
		$limit = apply_filters( 'zinn_cache_pro_imgoptm_destroy_max_rows', 500 );
		...
		$msg = __( 'Destroy all optimization data successfully.', 'zinn-cache-pro' );
		Admin_Display::success( $msg );
	}
	...
	private function _rescan() {
		...
		if ( ! $list ) {
			$msg = __( 'Rescanned successfully.', 'zinn-cache-pro' );
			Admin_Display::success( $msg );
		...
		$msg = $count ? sprintf( __( 'Rescanned %d images successfully.', 'zinn-cache-pro' ), $count ) : __( 'Rescanned successfully.', 'zinn-cache-pro' );
		Admin_Display::success( $msg );
	...
	private function _calc_bkup() {
		...
		$msg = __( 'Calculated backups successfully.', 'zinn-cache-pro' );
		Admin_Display::success( $msg );
	}
	...
	public function rm_bkup() {
		...
		$msg = __( 'Removed backups successfully.', 'zinn-cache-pro' );
		Admin_Display::success( $msg );
	}
	...
	public function batch_switch( $type ) {
		...
		$msg = __( 'Switched images successfully.', 'zinn-cache-pro' );
		Admin_Display::success( $msg );
	}
	...
	private function _switch_optm_file( $type ) {
		Admin_Display::success( __( 'Switched to optimized file successfully.', 'zinn-cache-pro' ) );
		return;
		...
		foreach ( $list as $v ) {
			...
			if ( 'webp' === $switch_type ) {
				if ( $this->__media->info( $v->src . '.webp', $v->post_id ) ) {
					...
					$msg = __( 'Disabled WebP file successfully.', 'zinn-cache-pro' );
					...
					$msg = __( 'Enabled WebP file successfully.', 'zinn-cache-pro' );
				}
				...
				if ( $this->__media->info( $v->src . '.avif', $v->post_id ) ) {
					...
					$msg = __( 'Disabled AVIF file successfully.', 'zinn-cache-pro' );
					...
					$msg = __( 'Enabled AVIF file successfully.', 'zinn-cache-pro' );
				}
			} else {
				...
				if ( $this->__media->info( $bk_file, $v->post_id ) ) {
					...
					$msg = __( 'Restored original file successfully.', 'zinn-cache-pro' );
					...
					$msg = __( 'Switched to optimized file successfully.', 'zinn-cache-pro' );
				}
			}
		}
	...
	public function reset_row( $post_id, $silent = false ) {
		...
		if ( ! $silent ) {
			$msg = __( 'Reset the optimized data successfully.', 'zinn-cache-pro' );
			Admin_Display::success( $msg );
		}
	}
