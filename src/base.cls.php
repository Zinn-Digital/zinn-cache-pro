...
class Base extends Root {
	...
	const OPTION_NAME = 'zinn-cache-pro-conf';
	...
	public function load_default_site_vals() {
		...
		if ( file_exists( ZINN_CACHE_PRO_DIR . 'data/const.network_default.json' ) ) {
			$default_ini_cfg = json_decode( File::read( ZINN_CACHE_PRO_DIR . 'data/const.network_default.json' ), true );
	...
	public function load_default_vals() {
		// Load default.json
		if ( file_exists( ZINN_CACHE_PRO_DIR . 'data/const.default.json' ) ) {
			$default_ini_cfg = json_decode( File::read( ZINN_CACHE_PRO_DIR . 'data/const.default.json' ), true );
		...
		if ( ! self::$_default_options[ self::O_CDN_ORI_DIR ] ) {
			self::$_default_options[ self::O_CDN_ORI_DIR ] = ZINN_CACHE_PRO_CONTENT_FOLDER . "\nwp-includes";
	...
	public static function conf_const( $id ) {
		return 'ZINN_CACHE_PRO_CONF__' . strtoupper( str_replace( '-', '__', $id ) );
	}
	...
	public function server_vars() {
		$consts      = [
			...
			'WP_CONTENT_DIR',
			'SHORTINIT',
			'ZINN_CACHE_PRO_CONTENT_DIR',
			'ZINN_CACHE_PRO_CONTENT_FOLDER',
			'ZINN_CACHE_PRO_DIR',
			'ZINN_CACHE_PRO_TIME_OFFSET',
			'ZINN_CACHE_PRO_SERVER_TYPE',
			'ZINN_CACHE_PRO_CLI',
			'ZINN_CACHE_PRO_ALLOWED',
			'ZINN_CACHE_PRO_ON',
			'ZINN_CACHE_PRO_TAG_PREFIX',
			'COOKIEHASH',
		];
		$server_vars = [];
		foreach ( $consts as $v ) {
	...
	protected function _save_css_con( $type, $css, $url_tag, $vary, $queue_k, $is_mobile, $is_webp ) {
		$css = apply_filters( 'zinn_cache_pro_' . $type, $css, $queue_k );
		// Font optimize
		...
		$static_file     = ZINN_CACHE_PRO_STATIC_DIR . $filepath_prefix . $filecon_md5 . '.css';
