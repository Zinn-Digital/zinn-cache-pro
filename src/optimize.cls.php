...
class Optimize extends Base {
	...
	const DUMMY_CSS_REGEX = "#<link [ \w='\"/]*id=['\"]zinn-cache-pro-dummy-css['\"] href=['\"].+assets/css/litespeed-dummy\.css[?\w.=-]*['\"][ \w='\"/]*>#isU";
	...
	public function init() {
		$this->cfg_css_async = defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_CSS_ASYNC);
		if ($this->cfg_css_async) {
			if (!$this->cls('Cloud')->activated()) {
				self::debug('❌ CCSS set to OFF due to QC not activated');
				$this->cfg_css_async = false;
			}
			if ((defined('ZINN_CACHE_PRO_GUEST_OPTM') || ($this->conf(self::O_OPTM_UCSS) && $this->conf(self::O_OPTM_CSS_COMB))) && $this->conf(self::O_OPTM_UCSS_INLINE)) {
				self::debug('⚠️ CCSS set to OFF due to UCSS Inline');
				$this->cfg_css_async = false;
			}
		}
		$this->cfg_js_defer = $this->conf(self::O_OPTM_JS_DEFER);
		if (defined('ZINN_CACHE_PRO_GUEST_OPTM')) {
			$this->cfg_js_defer = 2;
		}
		if ($this->cfg_js_defer == 2) {
			add_filter(
				'zinn_cache_pro_optm_cssjs',
		...
		if (defined('ZINN_CACHE_PRO_GUEST_OPTM')) {
			$this->cfg_js_defer_exc = apply_filters('zinn_cache_pro_optm_gm_js_exc', $this->conf(self::O_OPTM_GM_JS_EXC));
		} else {
			/**
			...
			if ($this->cfg_js_defer) {
				add_filter('zinn_cache_pro_optm_js_defer_exc', array( $this->cls('Data'), 'load_js_defer_exc' ));
				$this->cfg_js_defer_exc = apply_filters('zinn_cache_pro_optm_js_defer_exc', $this->conf(self::O_OPTM_JS_DEFER_EXC));

				$this->cfg_js_delay_inc = apply_filters('zinn_cache_pro_optm_js_delay_inc', $this->conf(self::O_OPTM_JS_DELAY_INC));
			}
		}

		// Add vary filter for Role Excludes @since  1.6
		add_filter('zinn_cache_pro_vary', array( $this, 'vary_add_role_exclude' ));

		// DNS optm (Prefetch/Preconnect) @since 7.3
		$this->_dns_optm_init();

		add_filter('zinn_cache_pro_buffer_finalize', array( $this, 'finalize' ), 20);

		// Inject a dummy CSS file to control final optimized data location in <head>
		wp_enqueue_style(Core::PLUGIN_NAME . '-dummy', ZINN_CACHE_PRO_PLUGIN_URL . 'assets/css/litespeed-dummy.css');
	}
	...
	public function rm_cache_folder( $subsite_id = false ) {
		if ($subsite_id) {
			file_exists(ZINN_CACHE_PRO_STATIC_DIR . '/css/' . $subsite_id) && File::rrmdir(ZINN_CACHE_PRO_STATIC_DIR . '/css/' . $subsite_id);
			file_exists(ZINN_CACHE_PRO_STATIC_DIR . '/js/' . $subsite_id) && File::rrmdir(ZINN_CACHE_PRO_STATIC_DIR . '/js/' . $subsite_id);
			return;
		}

		file_exists(ZINN_CACHE_PRO_STATIC_DIR . '/css') && File::rrmdir(ZINN_CACHE_PRO_STATIC_DIR . '/css');
		file_exists(ZINN_CACHE_PRO_STATIC_DIR . '/js') && File::rrmdir(ZINN_CACHE_PRO_STATIC_DIR . '/js');
	}
	...
	public function remove_query_strings( $src ) {
		if (strpos($src, '_zinn_cache_pro_rm_qs=0') || strpos($src, '/recaptcha')) {
			return $src;
		}
	...
	private function _finalize( $content ) {
		if (defined('ZINN_CACHE_PRO_NO_PAGEOPTM')) {
			self::debug2('bypass: NO_PAGEOPTM const');
			return $content;
		}

		if (!defined('ZINN_CACHE_PRO_IS_HTML')) {
		...
		if (!defined('ZINN_CACHE_PRO_GUEST_OPTM')) {
			...
			add_filter('zinn_cache_pro_optm_uri_exc', array( $this->cls('Data'), 'load_optm_uri_exc' ));
			$excludes = apply_filters('zinn_cache_pro_optm_uri_exc', $this->conf(self::O_OPTM_EXC));
	...
	private function _optimize() {
		...
		$this->cfg_css_min            = defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_CSS_MIN);
		$this->cfg_css_comb           = defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_CSS_COMB);
		$this->cfg_js_min             = defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_JS_MIN);
		$this->cfg_js_comb            = defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_JS_COMB);
		$this->cfg_ggfonts_rm         = defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_GGFONTS_RM);
		$this->cfg_ggfonts_async      = !defined('ZINN_CACHE_PRO_GUEST_OPTM') && $this->conf(self::O_OPTM_GGFONTS_ASYNC); // forced rm already
		$this->_conf_css_font_display = !defined('ZINN_CACHE_PRO_GUEST_OPTM') && $this->conf(self::O_OPTM_CSS_FONT_DISPLAY);
		...
		if ($this->cfg_css_min || $this->cfg_css_comb || $this->cfg_ggfonts_rm || $this->cfg_css_async || $this->cfg_ggfonts_async || $this->_conf_css_font_display) {
			add_filter('zinn_cache_pro_optimize_css_excludes', array( $this->cls('Data'), 'load_css_exc' ));
		...
		if ($this->cfg_css_min || $this->cfg_css_comb) {
			if ($src_list) {
				...
				if ($this->cfg_css_comb) {
					...
					if ((defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_UCSS)) && $this->conf(self::O_OPTM_UCSS_INLINE)) {
						$filename = $this->cls('UCSS')->load($this->_request_url, true);
						if ($filename) {
							$filepath_prefix = $this->_build_filepath_prefix('ucss');
							$this->_ucss     = File::read(ZINN_CACHE_PRO_STATIC_DIR . $filepath_prefix . $filename);
		...
		if ($this->cfg_js_min || $this->cfg_js_comb || $this->cfg_js_defer || $this->cfg_js_delay_inc) {
			add_filter('zinn_cache_pro_optimize_js_excludes', array( $this->cls('Data'), 'load_js_exc' ));
		...
		if ($this->cfg_css_async) {
			...
			if ($this->conf(self::O_OPTM_CSS_ASYNC_INLINE)) {
				$this->html_head .= $this->_build_js_inline(File::read(ZINN_CACHE_PRO_DIR . self::LIB_FILE_CSS_ASYNC), true);
			} else {
				$css_async_lib_url = ZINN_CACHE_PRO_PLUGIN_URL . self::LIB_FILE_CSS_ASYNC;
		...
		$this->html_head_early = apply_filters('zinn_cache_pro_optm_html_head_early', $this->html_head_early);
		...
		$this->html_head = apply_filters('zinn_cache_pro_optm_html_head', $this->html_head);
		if ($this->html_head) {
			if (apply_filters('zinn_cache_pro_optm_html_after_head', false)) {
		...
		$this->html_foot = apply_filters('zinn_cache_pro_optm_html_foot', $this->html_foot);
		if ($this->html_foot) {
		...
		// HTML minify
		if (defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_HTML_MIN)) {
	...
	private function _build_js_tag( $src ) {
		if ($this->cfg_js_defer === 2 || Utility::str_hit_array($src, $this->cfg_js_delay_inc)) {
			return '<script data-optimized="1" type="zinn-cache-pro/javascript" data-src="' . Str::trim_quotes($src) . '"></script>';
		}
	...
	private function _maybe_js_delay() {
		...
		if (!defined('ZINN_CACHE_PRO_JS_DELAY_LIB_LOADED')) {
			define('ZINN_CACHE_PRO_JS_DELAY_LIB_LOADED', true);
			$this->html_foot .= '<script>' . File::read(ZINN_CACHE_PRO_DIR . self::LIB_FILE_JS_DELAY) . '</script>';
		}
	}
	...
	private function _async_ggfonts() {
		...
		$webfont_lib_url = ZINN_CACHE_PRO_PLUGIN_URL . self::LIB_FILE_WEBFONTLOADER;
	...
	private function _dns_optm_init() {
		...
		if (defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(self::O_OPTM_DNS_PREFETCH_CTRL)) {
		...
		if (function_exists('wp_resource_hints')) {
			...
			add_action('zinn_cache_pro_optm', array( $this, 'dns_optm_output' ));
		}
	}
	...
	private function _build_hash_url( $src_list, $file_type = 'css' ) {
		...
		return ZINN_CACHE_PRO_STATIC_URL . $filepath_prefix . $filename . '?ver=' . $qs_hash;
	}
	...
	private function _parse_js() {
		$excludes = apply_filters('zinn_cache_pro_optimize_js_excludes', $this->conf(self::O_OPTM_JS_EXC));

		$combine_ext_inl = $this->conf(self::O_OPTM_JS_COMB_EXT_INL);
		if (!apply_filters('zinn_cache_pro_optm_js_comb_ext_inl', true)) {
			self::debug2('js_comb_ext_inl bypassed via zinn_cache_pro_optm_js_comb_ext_inl filter');
	...
	private function _js_inline_defer( $con, $attrs = false, $minified = false ) {
		...
		if ($this->cfg_js_defer === 2) {
			...
			$con = str_replace('DOMContentLoaded', 'DOMContentLiteSpeedLoaded', $con);
			return '<script' . $attrs . ' type="zinn-cache-pro/javascript">' . $con . '</script>';
			// return '<script' . $attrs . ' type="zinn-cache-pro/javascript" src="data:text/javascript;base64,' . base64_encode( $con ) . '"></script>';
			// return '<script' . $attrs . ' type="zinn-cache-pro/javascript">' . $con . '</script>';
		}
	...
	private function _preserve_esi( $con ) {
		...
		foreach ($esi_placeholder_list as $esi_placeholder) {
			$js_var                   = '__zinn_cache_pro_var_' . self::$_var_i++ . '__';
	...
	private function _parse_css() {
		$excludes             = apply_filters('zinn_cache_pro_optimize_css_excludes', $this->conf(self::O_OPTM_CSS_EXC));
		$ucss_file_exc_inline = apply_filters('zinn_cache_pro_optimize_ucss_file_exc_inline', $this->conf(self::O_OPTM_UCSS_FILE_EXC_INLINE));

		// Append dummy css to exclude list
		$excludes[] = 'litespeed-dummy.css';

		$combine_ext_inl = $this->conf(self::O_OPTM_CSS_COMB_EXT_INL);
		if (!apply_filters('zinn_cache_pro_optm_css_comb_ext_inl', true)) {
			self::debug2('css_comb_ext_inl bypassed via zinn_cache_pro_optm_css_comb_ext_inl filter');
			$combine_ext_inl = false;
		}

		$css_to_be_removed = apply_filters('zinn_cache_pro_optm_css_to_be_removed', array());
	...
	private function _async_css( $ori ) {
		...
		if (!defined('ZINN_CACHE_PRO_GUEST_OPTM') && !$this->conf(self::O_OPTM_NOSCRIPT_RM)) {
	...
	private function _js_defer( $ori, $src ) {
		...
		if ($this->cfg_js_defer === 2 || Utility::str_hit_array($src, $this->cfg_js_delay_inc)) {
			...
			return str_replace(' src=', ' type="zinn-cache-pro/javascript" data-src=', $ori);
		}
