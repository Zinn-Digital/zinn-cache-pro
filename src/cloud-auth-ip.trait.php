...
trait Cloud_Auth_IP {
	...
	public function is_from_cloud() {
		// Nothing is "from cloud" any more: the QUIC.cloud coupling is removed, so every
		// cloud-authenticated REST callback fails closed. Upstream's ip_access() returns
		// false on an empty allowlist, so this is the same denial value callers expect.
		return false;
	...
	private function _update_ips() {
		// Removed: the remote QUIC.cloud IP allowlist fetch. Record an empty list so
		// Router::ip_access() fails closed and callers do not re-enter this method.
		self::save_summary([
				'ips'           => [],
				'ips_ts'        => time(),
				'ips_ts_runner' => time(),
