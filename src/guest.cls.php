...
class Guest extends Cloud {
	...
	public function sync_lists() {
		...
		$cloud_dir = ZINN_CACHE_PRO_STATIC_DIR . '/cloud';
		...
		$message = $success
			? __( 'Guest Mode lists synced successfully.', 'zinn-cache-pro' )
			: __( 'Failed to sync Guest Mode lists.', 'zinn-cache-pro' );
	...
	private function _fetch_api( $url ) {
		// Removed: Guest Mode IP/UA lists were fetched from wpapi.quic.cloud on a daily
		// cron. false is this method's documented failure value; sync_lists() reports it.
		unset( $url );
		self::debug( 'Guest Mode list sync is disabled in this build' );
		return false;
