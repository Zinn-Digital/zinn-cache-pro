 ...
 * @return bool
 */
function zinn_cache_pro_table_exists( $table_name ) {
	global $wpdb;
 ...
 * @return void
 */
function zinn_cache_pro_update_7_0_1() {
	global $wpdb;

	Debug2::debug( '[Data] v7.0.1 upgrade started' );

	$tb_url = $wpdb->prefix . 'zinn_cache_pro_url';
	if ( ! zinn_cache_pro_table_exists( $tb_url ) ) {
		Debug2::debug( '[Data] Table `zinn_cache_pro_url` not found, bypassed migration' );
		return;
	}
 ...
 * @since 7.0
 */
function zinn_cache_pro_update_7() {
 ...
 * @since 7.7
 */
function zinn_cache_pro_update_7_7() {
	global $wpdb;
	...
	$tb_url = $wpdb->prefix . 'zinn_cache_pro_url';
	if ( ! zinn_cache_pro_table_exists( $tb_url ) ) {
		Debug2::debug( '[Data] Table `zinn_cache_pro_url` not found, bypassed URL migration' );
		return;
	}
 ...
 * @since 5.3
 */
function zinn_cache_pro_update_5_3() {
	global $wpdb;
	Debug2::debug('[Data] Upgrade url_file table');

	$tb = $wpdb->prefix . 'zinn_cache_pro_url_file';
	if ( zinn_cache_pro_table_exists( $tb ) ) {
		$q = "ALTER TABLE `{$tb}`
