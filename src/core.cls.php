...
class Core extends Root {
	...
	public function __construct() {
		! defined( 'ZINN_CACHE_PRO_TS_0' ) && define( 'ZINN_CACHE_PRO_TS_0', microtime( true ) );
		$this->cls( 'Conf' )->init();
		...
		if ( defined( 'ZINN_CACHE_PRO_ON' ) ) {
			// Load third party detection if lscache enabled.
			include_once ZINN_CACHE_PRO_DIR . 'thirdparty/entry.inc.php';
		}
		 ...
		 */
		$plugin_file = ZINN_CACHE_PRO_DIR . 'zinn-cache-pro.php';
		...
		register_uninstall_hook( $plugin_file, __NAMESPACE__ . '\Activation::uninstall_zinn_cache_pro_cache' );
		...
		if ( $this->cls( 'Router' )->esi_enabled() && ! function_exists( 'wp_create_nonce' ) ) {
			...
			zinn_cache_pro_define_nonce_func();
		}
	}
	...
	public function init() {
		 ...
		 */
		do_action( 'zinn_cache_pro_init' );
		add_action( 'wp_ajax_async_zinn_cache_pro', 'ZinnCachePro\Task::async_zinn_cache_pro_handler' );
		add_action( 'wp_ajax_nopriv_async_zinn_cache_pro', 'ZinnCachePro\Task::async_zinn_cache_pro_handler' );
		...
		do_action( 'zinn_cache_pro_initing' );
		...
		if ( ! empty( $_GET[ Router::ACTION ] ) && 'before_optm' === $_GET[ Router::ACTION ] && ! apply_filters( 'zinn_cache_pro_qs_forbidden', false ) ) {
	...
	public function after_user_init() {
		...
		if ( ! is_admin() && ! defined( 'ZINN_CACHE_PRO_GUEST_OPTM' ) ) {
		...
		if ( ! defined( 'ZINN_CACHE_PRO_NO_OPTM' ) || ! ZINN_CACHE_PRO_NO_OPTM ) {
	...
	public function proceed_action( $action ) {
		...
		switch ( $action ) {
			...
			case self::ACTION_QS_PURGE_EMPTYCACHE:
				define( 'ZINN_CACHE_PRO_EMPTYCACHE', true ); // Clear all sites caches
				Purge::purge_all();
				$msg = __( 'Notified LiteSpeed Web Server to purge everything.', 'zinn-cache-pro' );
				break;

			case self::ACTION_PURGE_BY:
				$this->cls( 'Purge' )->purge_list();
				$msg = __( 'Notified LiteSpeed Web Server to purge the list.', 'zinn-cache-pro' );
				break;
	...
	public function load_thirdparty() {
		do_action( 'zinn_cache_pro_load_thirdparty' );
	}
	...
	public function footer_hook() {
		...
		if ( ! defined( 'ZINN_CACHE_PRO_FOOTER_CALLED' ) ) {
			define( 'ZINN_CACHE_PRO_FOOTER_CALLED', true );
		}
	}
	...
	private function check_is_html( $buffer = null ) {
		...
		if ( ! defined( 'ZINN_CACHE_PRO_IS_HTML' ) ) {
			define( 'ZINN_CACHE_PRO_IS_HTML', true );
		}
	}
	...
	public function send_headers_force( $buffer ) {
		...
		$buffer = apply_filters( 'zinn_cache_pro_buffer_before', $buffer );
		 ...
		 */
		if ( ! defined( 'ZINN_CACHE_PRO_NO_OPTM' ) || ! ZINN_CACHE_PRO_NO_OPTM ) {
			Debug2::debug( '[Core] run hook zinn_cache_pro_buffer_finalize' );
			$buffer = apply_filters( 'zinn_cache_pro_buffer_finalize', $buffer );
		}
		...
		// Init comment info
		$running_info_showing = defined( 'ZINN_CACHE_PRO_IS_HTML' ) || defined( 'LSCACHE_IS_ESI' );
		...
		$running_info_showing = apply_filters( 'zinn_cache_pro_comment', $running_info_showing );
		...
		if ( apply_filters( 'zinn_cache_pro_is_json', false ) ) {
		...
		$buffer = apply_filters( 'zinn_cache_pro_buffer_after', $buffer );
	...
	public function send_headers( $is_forced = false ) {
		...
		// Cache control output needs to be done first, as some varies are added in 3rd party hook `zinn_cache_pro_api_control`.
		...
		// Send Control header
		if ( defined( 'ZINN_CACHE_PRO_ON' ) && $control_header ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_ON' ) && $purge_header ) {
		...
		// Send Vary header
		if ( defined( 'ZINN_CACHE_PRO_ON' ) && $vary_header ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_ON' ) && defined( 'ZINN_CACHE_PRO_LOG' ) ) {
		...
		} elseif ( defined( 'ZINN_CACHE_PRO_ON' ) && Control::is_cacheable() && $tag_header ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_LOG' ) && defined( 'ZINN_CACHE_PRO_OBJECT_CACHE' ) && method_exists( 'WP_Object_Cache', 'debug' ) ) {
			$this->comment( 'Object Cache ' . \WP_Object_Cache::get_instance()->debug() );
		}

		if ( defined( 'ZINN_CACHE_PRO_GUEST' ) && ZINN_CACHE_PRO_GUEST ) {
