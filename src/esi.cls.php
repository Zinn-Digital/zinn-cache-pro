...
class ESI extends Root {
	...
	public function init() {
		...
		if (defined('DONOTCACHEPAGE') && apply_filters('zinn_cache_pro_const_DONOTCACHEPAGE', DONOTCACHEPAGE)) {
			return;
		}

		// Init ESI in `after_setup_theme` hook after detected if ZINN_CACHE_PRO_DISABLE_ALL is ON or not
		$this->_hooks();
		...
		!defined('ZINN_CACHE_PRO_ESI_INITED') && define('ZINN_CACHE_PRO_ESI_INITED', true);
	}

	/**
	 * Init ESI related hooks
	 *
	 * Load delayed by hook to give the ability to bypass by ZINN_CACHE_PRO_DISABLE_ALL const
	 *
	 * @since 2.9.7.2
	...
	private function _transform_nonce() {
		...
		add_filter('zinn_cache_pro_esi_nonces', array( $this->cls('Data'), 'load_esi_nonces' ));
		if ($nonces = apply_filters('zinn_cache_pro_esi_nonces', $nonces)) {
			foreach ($nonces as $action) {
				$this->nonce_action($action);
			}
		}

		add_action('zinn_cache_pro_nonce', array( $this, 'nonce_action' ));
	}
	...
	public function is_nonce_action( $action ) {
		...
		if (!defined('ZINN_CACHE_PRO_ESI_INITED')) {
			return null;
		}
	...
	public function shortcode( $atts ) {
		...
		do_action('zinn_cache_pro_esi_shortcode-' . $atts[0]);
	...
	private function _register_esi_actions() {
		...
		!empty($_SERVER['ESI_REFERER']) && defined('ZINN_CACHE_PRO_LOG') && Debug2::debug('[ESI] ESI_REFERER: ' . $_SERVER['ESI_REFERER']);
		...
		if (!empty($_SERVER['ESI_CONTENT_TYPE']) && strpos($_SERVER['ESI_CONTENT_TYPE'], 'application/json') === 0) {
			add_filter('zinn_cache_pro_is_json', '__return_true');
		}
		...
		// Register ESI blocks
		add_action('zinn_cache_pro_esi_load-widget', array( $this, 'load_widget_block' ));
		add_action('zinn_cache_pro_esi_load-admin-bar', array( $this, 'load_admin_bar_block' ));
		add_action('zinn_cache_pro_esi_load-comment-form', array( $this, 'load_comment_form_block' ));

		add_action('zinn_cache_pro_esi_load-nonce', array( $this, 'load_nonce_block' ));
		add_action('zinn_cache_pro_esi_load-esi', array( $this, 'load_esi_shortcode' ));

		add_action('zinn_cache_pro_esi_load-' . self::COMBO, array( $this, 'load_combo' ));
	}
	...
	public function esi_template( $template ) {
		...
		if (defined('LSCACHE_IS_ESI')) {
			...
			return ZINN_CACHE_PRO_DIR . 'tpl/esi.tpl.php';
		}
	...
	private function _register_not_esi_actions() {
		do_action('zinn_cache_pro_tpl_normal');
	...
	public function load_combo() {
		...
		foreach ($_POST['esi_include'] as $url) {
			...
			$inline_param = apply_filters('zinn_cache_pro_esi_inline-' . $esi_id, array(), $esi_param); // Returned array need to be [ val, control, tag ]
			if ($inline_param) {
	...
	public function sub_esi_block(
		...
		if (defined('ZINN_CACHE_PRO_ESI_OFF')) {
			...
			do_action('zinn_cache_pro_esi_load-' . $block_id, $params);
			return;
		}
		...
		$params  = apply_filters('zinn_cache_pro_esi_params', $params, $block_id);
		$control = apply_filters('zinn_cache_pro_esi_control', $control, $block_id);

		if (!is_array($params) || !is_string($control)) {
			defined('ZINN_CACHE_PRO_LOG') && Debug2::debug("[ESI] 🛑 Sub hooks returned Params: \n" . var_export($params, true) . "\ncache control: \n" . var_export($control, true));
	...
	public function load_esi_block() {
		...
		do_action('zinn_cache_pro_esi_load-' . LSCACHE_IS_ESI, $params);
	}
	...
	public function sub_widget_block( $instance, $widget, $args ) {
		...
		if (!isset($options) || !$options[self::WIDGET_O_ESIENABLE]) {
			defined('ZINN_CACHE_PRO_LOG') && Debug2::debug('ESI 0 ' . $name . ': ' . (!isset($options) ? 'not set' : 'set off'));
	...
	public function load_widget_block( $params ) {
		...
		defined('ZINN_CACHE_PRO_LOG') && Debug2::debug('ESI widget render: name ' . $params[self::PARAM_NAME] . ', id ' . $params[self::PARAM_ID] . ', ttl ' . $ttl);
		if ($ttl == 0) {
	...
	public function load_admin_bar_block( $params ) {
		...
		defined('ZINN_CACHE_PRO_LOG') && Debug2::debug('ESI: adminbar ref: ' . $_SERVER['REQUEST_URI']);
	}
	...
	public function load_nonce_block( $params ) {
		...
		if (function_exists('wp_create_nonce_zinn_cache_pro_esi')) {
			echo wp_create_nonce_zinn_cache_pro_esi($action);
		} else {
