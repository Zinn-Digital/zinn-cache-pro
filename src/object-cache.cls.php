...
class Object_Cache extends Root {
	...
	public function __construct( $cfg = false ) {
		...
		} elseif ( defined( 'ZINN_CACHE_PRO_CONF_LOADED' ) ) { // If OC is OFF, will hit here to init OC after conf initialized
		...
		if ( ! $this->_cfg_enabled ) {
			! defined( 'ZINN_CACHE_PRO_OC_FAILURE' ) && define( 'ZINN_CACHE_PRO_OC_FAILURE', true );
		}
	}
	...
	private function debug_oc( $text ) {
		...
		$zinn_cache_pro_data_folder = defined( 'ZINN_CACHE_PRO_DATA_FOLDER' ) ? ZINN_CACHE_PRO_DATA_FOLDER : 'zinn-cache-pro';
		$lscwp_content_dir     = defined( 'ZINN_CACHE_PRO_CONTENT_DIR' ) ? ZINN_CACHE_PRO_CONTENT_DIR : WP_CONTENT_DIR;
		$zinn_cache_pro_static_dir  = $lscwp_content_dir . '/' . $zinn_cache_pro_data_folder;
		$log_path_prefix       = $zinn_cache_pro_static_dir . '/debug/';
	...
	public function update_file( $options ) {
		...
		// NOTE: When included in oc.php, `ZINN_CACHE_PRO_DIR` will show undefined, so this must be assigned/generated when used.
		$_oc_ori_file = ZINN_CACHE_PRO_DIR . 'lib/object-cache.php';
	...
	private function _connect() {
		...
		if ( defined( 'ZINN_CACHE_PRO_OC_FAILURE' ) ) {
			$this->debug_oc( 'ZINN_CACHE_PRO_OC_FAILURE const defined' );
			return false;
		}
		...
		if ( 'Redis' === $this->_oc_driver ) {
			...
			set_error_handler( 'zinn_cache_pro_exception_handler' );
			try {
		...
		if ( $failed ) {
			...
			if ( function_exists( 'did_action' ) && did_action( 'muplugins_loaded' ) ) {
				zinn_cache_pro_oc_disable_ext_cache();
			} elseif ( function_exists( 'add_action' ) ) {
				add_action( 'muplugins_loaded', 'zinn_cache_pro_oc_disable_ext_cache', -999 );
			}
	...
	public function set( $key, $data, $expire ) {
		...
		if ( 'Redis' === $this->_oc_driver ) {
			try {
				...
				$res = false;
				$msg = sprintf( __( 'Redis encountered a fatal error: %1$s (code: %2$d)', 'zinn-cache-pro' ), $ex->getMessage(), $ex->getCode() );
				$this->debug_oc( $msg );
