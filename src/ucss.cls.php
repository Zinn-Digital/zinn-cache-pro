...
class UCSS extends Base {
	...
	public function __construct() {
		...
		add_filter( 'zinn_cache_pro_ucss_whitelist', [ $this->cls( 'Data' ), 'load_ucss_whitelist' ] );
	}
	...
	public static function get_url_tag( $request_url = false ) {
		...
		if (is_404()) {
			$url_tag = '404';
		} elseif (apply_filters('zinn_cache_pro_ucss_per_pagetype', false)) {
			$url_tag = Utility::page_type();
			self::debug('zinn_cache_pro_ucss_per_pagetype filter altered url to ' . $url_tag);
		}
	...
	public function load( $request_url, $dry_run = false ) {
		...
		$ucss_exc = apply_filters( 'zinn_cache_pro_ucss_exc', $this->conf( self::O_OPTM_UCSS_EXC ) );
		...
		if ($filename) {
			$static_file = ZINN_CACHE_PRO_STATIC_DIR . $filepath_prefix . $filename . '.css';
		...
		$this->_queue[ $queue_k ] = [
			'url'        => apply_filters( 'zinn_cache_pro_ucss_url', $request_url ),
	...
	private function _filter_whitelist() {
		$whitelist = [];
		$list      = apply_filters('zinn_cache_pro_ucss_whitelist', $this->conf(self::O_OPTM_UCSS_SELECTOR_WHITELIST));
		foreach ($list as $k => $v) {
			if (substr($v, 0, 2) === '//') {
				continue;
			}
