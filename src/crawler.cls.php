...
class Crawler extends Root {
	...
	public function __construct() {
		...
		$this->_resetfile = ZINN_CACHE_PRO_STATIC_DIR . '/crawler/' . $this->_sitemeta . '.reset';
	...
	public function clear_disabled_list() {
		...
		$msg = __( 'Crawler disabled list is cleared! All crawlers are set to active! ', 'zinn-cache-pro' );
	...
	public static function save_summary( $data = false, $reload = false, $overwrite = false ) {
		...
		File::save( ZINN_CACHE_PRO_STATIC_DIR . '/crawler/' . $instance->_sitemeta, wp_json_encode( $data ), true );
	}
	...
	public static function start_async() {
		...
		$msg = __( 'Started async crawling', 'zinn-cache-pro' );
	...
	private function _crawl_data( $manually_run ) {
		if ( ! defined( 'ZINN_CACHE_PRO_LANE_HASH' ) ) {
			define( 'ZINN_CACHE_PRO_LANE_HASH', Str::rrand( 8 ) );
		}
	...
	private function load_conf() {
		...
		if ( defined( 'ZINN_CACHE_PRO_CRAWLER_USLEEP' ) && constant( 'ZINN_CACHE_PRO_CRAWLER_USLEEP' ) > $this->_crawler_conf['run_delay'] ) {
			$this->_crawler_conf['run_delay'] = (int) constant( 'ZINN_CACHE_PRO_CRAWLER_USLEEP' );
		}
		...
		if ( ! empty( $current_crawler['uid'] ) ) {
			...
			$this->_crawler_conf['cookies']['zinn_cache_pro_hash'] = Router::cls()->get_hash( $current_crawler['uid'] );
		}
	...
	public function get_crawler_duration() {
		$run_duration = defined( 'ZINN_CACHE_PRO_CRAWLER_DURATION' ) ? (int) constant( 'ZINN_CACHE_PRO_CRAWLER_DURATION' ) : 900;
		if ( $run_duration > 900 ) {
	...
	private function _adjust_current_threads() {
		...
		$crawler_threads = defined( 'ZINN_CACHE_PRO_CRAWLER_THREADS' ) ? (int) constant( 'ZINN_CACHE_PRO_CRAWLER_THREADS' ) : 3;
	...
	private function _take_over_lane() {
		...
		File::save( $this->json_local_path() . '.pid', ZINN_CACHE_PRO_LANE_HASH );
	}
	...
	private function _check_valid_lane( $strict_mode = false ) {
		...
		if ( $pid && ZINN_CACHE_PRO_LANE_HASH !== $pid ) {
	...
	private function _test_port() {
		...
		if ( defined( 'ZINN_CACHE_PRO_CRAWLER_LOCAL_PORT' ) ) {
			self::debug( '✅ ZINN_CACHE_PRO_CRAWLER_LOCAL_PORT already defined' );
			return true;
		}
		...
		if ( ! empty( $this->_summary['test_port_tts'] ) && ( time() - (int) $this->_summary['test_port_tts'] ) < 120 ) {
			if ( ! empty( $this->_summary['test_port'] ) ) {
				...
				define( 'ZINN_CACHE_PRO_CRAWLER_LOCAL_PORT', (int) $this->_summary['test_port'] );
				return true;
			}
			return false;
		}
		...
		$home    = home_url();
		File::save( ZINN_CACHE_PRO_STATIC_DIR . '/crawler/test_port.html', $home, true );
		$url        = ZINN_CACHE_PRO_STATIC_URL . '/crawler/test_port.html';
		...
		if ( curl_errno( $ch ) || $result !== $home ) {
			...
			if ( curl_errno( $ch ) ) {
				...
				define( 'ZINN_CACHE_PRO_CRAWLER_LOCAL_PORT', 80 );
				$this->_summary['test_port'] = 80;
				$test_result                 = true;
			}
		} else {
			self::debug( '✅ Tested local 443 port successfully' );
			define( 'ZINN_CACHE_PRO_CRAWLER_LOCAL_PORT', 443 );
	...
	private function _should_force_resolve_dns() {
		...
		if ( ! empty( $this->_crawler_conf['cookies'] ) && ! empty( $this->_crawler_conf['cookies']['zinn_cache_pro_hash'] ) ) {
			return true;
		}
		return false;
	}
	...
	private function _multi_request( $rows, $options ) {
		...
		// phpcs:disable WordPress.WP.AlternativeFunctions
		$mh                  = curl_multi_init();
		$crawler_drop_domain = defined( 'ZINN_CACHE_PRO_CRAWLER_DROP_DOMAIN' ) ? (bool) constant( 'ZINN_CACHE_PRO_CRAWLER_DROP_DOMAIN' ) : false;
		$curls               = [];
		foreach ( $rows as $row ) {
			...
			if ( $this->_should_force_resolve_dns() ) {
				...
				if ( ! empty( $parsed_url['host'] ) ) {
					$dom                                     = $parsed_url['host'];
					$port                                    = defined( 'ZINN_CACHE_PRO_CRAWLER_LOCAL_PORT' ) ? (int) ZINN_CACHE_PRO_CRAWLER_LOCAL_PORT : 443;
	...
	private function _status_parse( $header, $code, $url ) {
		...
		if ( false !== stripos( $header, 'X-Litespeed-Cache-Control: no-cache' ) ) {
			...
			if ( defined( 'ZINN_CACHE_PRO_CRAWLER_IGNORE_NONCACHEABLE' ) && constant( 'ZINN_CACHE_PRO_CRAWLER_IGNORE_NONCACHEABLE' ) ) {
				return self::STATUS_MISS;
			}

			// If blacklist is disabled.
			if ( ( defined( 'ZINN_CACHE_PRO_CRAWLER_DISABLE_BLOCKLIST' ) && constant( 'ZINN_CACHE_PRO_CRAWLER_DISABLE_BLOCKLIST' ) ) || apply_filters( 'zinn_cache_pro_crawler_disable_blocklist', false, $url ) ) {
				return self::STATUS_MISS;
			}
	...
	private function _get_curl_options( $crawler_only = false ) {
		$crawler_timeout                 = defined( 'ZINN_CACHE_PRO_CRAWLER_TIMEOUT' ) ? (int) constant( 'ZINN_CACHE_PRO_CRAWLER_TIMEOUT' ) : 30;
		$options                         = [
	...
	public function self_curl( $url, $ua, $uid = false, $accept = false ) {
		...
		if ( $uid ) {
			$this->_crawler_conf['cookies']['zinn_cache_pro_flash_hash'] = Router::cls()->get_flash_hash( $uid );
			$parsed_url = wp_parse_url( $url );
	...
	public function list_crawlers() {
		...
		$crawler_factors['uid'] = [ 0 => __( 'Guest', 'zinn-cache-pro' ) ];

		// WebP on/off.
		if ( $this->conf( Base::O_IMG_OPTM_WEBP ) ) {
			$crawler_factors['webp'] = [ 1 => $this->cls( 'Media' )->next_gen_image_title() ];
			if ( apply_filters( 'zinn_cache_pro_crawler_webp', false ) ) {
	...
	public function json_local_path() {
		return ZINN_CACHE_PRO_STATIC_DIR . '/crawler/' . $this->_sitemeta;
	}
	...
	public function json_path() {
		if ( ! file_exists( ZINN_CACHE_PRO_STATIC_DIR . '/crawler/' . $this->_sitemeta ) ) {
			return false;
		}

		return ZINN_CACHE_PRO_STATIC_URL . '/crawler/' . $this->_sitemeta;
	}
	...
	public function display_status( $status_row, $reason_set ) {
		...
		foreach ( str_split( $status_row ) as $k => $v ) {
			...
			if ( 'Man' === $reason ) {
				$reason = __( 'Manually added to blocklist', 'zinn-cache-pro' );
			}
			if ( 'Existed' === $reason ) {
				$reason = __( 'Previously existed in blocklist', 'zinn-cache-pro' );
			}
