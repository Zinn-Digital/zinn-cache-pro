...
class Purge extends Base {
	...
	public function init() {
		$purge_post_events = apply_filters(
			'zinn_cache_pro_purge_post_events',
			[
				'delete_post',
				'wp_trash_post',
	...
	private function _purge_all( $reason = false ) {
		...
		$msg = __( 'Purged all caches successfully.', 'zinn-cache-pro' );
		if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
			Admin_Display::success( $msg );
		}

		do_action( 'zinn_cache_pro_purged_all' );
	}
	...
	private function _purge_all_lscache( $silence = false ) {
		...
		do_action( 'zinn_cache_pro_purged_all_lscache' );

		if ( ! $silence ) {
			$msg = __( 'Notified LiteSpeed Web Server to purge all LSCache entries.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	private function _purge_all_ccss( $silence = false ) {
		do_action( 'zinn_cache_pro_purged_all_ccss' );

		$this->cls( 'CSS' )->rm_cache_folder( 'ccss' );
		$this->cls( 'Data' )->url_file_clean( 'ccss' );

		if ( ! $silence ) {
			$msg = __( 'Cleaned all Critical CSS files.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	private function _purge_all_ucss( $silence = false ) {
		do_action( 'zinn_cache_pro_purged_all_ucss' );

		$this->cls( 'CSS' )->rm_cache_folder( 'ucss' );
		$this->cls( 'Data' )->url_file_clean( 'ucss' );

		if ( ! $silence ) {
			$msg = __( 'Cleaned all Unique CSS files.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	private function _purge_all_lqip( $silence = false ) {
		do_action( 'zinn_cache_pro_purged_all_lqip' );

		$this->cls( 'Placeholder' )->rm_cache_folder( 'lqip' );

		if ( ! $silence ) {
			$msg = __( 'Cleaned all LQIP files.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	private function _purge_all_vpi( $silence = false ) {
		global $wpdb;
		do_action( 'zinn_cache_pro_purged_all_vpi' );
		...
		if ( !$silence ) {
			$msg = __( 'Cleaned all VPI data.', 'zinn-cache-pro' );
			!defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) && Admin_Display::success( $msg );
		}
	}
	...
	private function _purge_all_avatar( $silence = false ) {
		do_action( 'zinn_cache_pro_purged_all_avatar' );
		...
		if ( ! $silence ) {
			$msg = __( 'Cleaned all Gravatar files.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	private function _purge_all_localres( $silence = false ) {
		do_action( 'zinn_cache_pro_purged_all_localres' );

		$this->_add( Tag::TYPE_LOCALRES );

		if ( ! $silence ) {
			$msg = __( 'Cleaned all localized resource entries.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	private function _purge_all_cssjs( $silence = false ) {
		if ( wp_doing_cron() || defined( 'ZINN_CACHE_PRO_DID_send_headers' ) ) {
		...
		do_action( 'zinn_cache_pro_purged_all_cssjs' );
		...
		if ( ! $silence ) {
			$msg = __( 'Notified LiteSpeed Web Server to purge CSS/JS entries.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	public function purge_all_opcache( $silence = false ) {
		if ( ! Router::opcache_enabled() ) {
			...
			if ( ! $silence ) {
				$msg = __( 'OPcache is not enabled.', 'zinn-cache-pro' );
				if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
		...
		if ( Router::opcache_restricted( __FILE__ ) ) {
			...
			if ( ! $silence ) {
				$msg = sprintf( __( 'OPcache is restricted by %s setting.', 'zinn-cache-pro' ), '<code>restrict_api</code>' );
				if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
					Admin_Display::error( $msg );
				}
			}
		...
		if ( ! opcache_reset() ) {
			...
			if ( ! $silence ) {
				$msg = __( 'Reset the OPcache failed.', 'zinn-cache-pro' );
				if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
					Admin_Display::success( $msg );
				}
			}
		...
		do_action( 'zinn_cache_pro_purged_all_opcache' );

		self::debug( 'Reset OPcache' );

		if ( ! $silence ) {
			$msg = __( 'Reset the entire OPcache successfully.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	private function _purge_all_object( $silence = false ) {
		if ( ! defined( 'ZINN_CACHE_PRO_OBJECT_CACHE' ) ) {
			...
			if ( ! $silence ) {
				$msg = __( 'Object cache is not enabled.', 'zinn-cache-pro' );
				Admin_Display::error( $msg );
			}
		...
		do_action( 'zinn_cache_pro_purged_all_object' );

		$this->cls( 'Object_Cache' )->flush();
		self::debug( 'Flushed object cache' );

		if ( ! $silence ) {
			$msg = __( 'Purge all object caches successfully.', 'zinn-cache-pro' );
			if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
				Admin_Display::success( $msg );
			}
		}
	...
	private function _add( $tags, $purge2 = false ) {
		...
		if ( defined( 'ZINN_CACHE_PRO_CLI' ) ) {
			...
			if ( wp_doing_cron() || defined( 'ZINN_CACHE_PRO_DID_send_headers' ) || apply_filters( 'zinn_cache_pro_delay_purge', false ) ) {
				self::update_option( $purge2 ? self::DB_QUEUE2 : self::DB_QUEUE, $curr_built );
	...
	public static function set_purge_single() {
		...
		do_action( 'zinn_cache_pro_purged_single' );
	}
	...
	private function _purge_front() {
		...
		do_action( 'zinn_cache_pro_purged_front', $ref );
	...
	private function _purge_ucss() {
		...
		do_action( 'zinn_cache_pro_purge_ucss', $url_tag );
		$this->purge_url( $ref );
	...
	private function _purge_frontpage() {
		...
		if ( 'ZINN_CACHE_PRO_SERVER_OLS' !== ZINN_CACHE_PRO_SERVER_TYPE ) {
			$this->_add_private( Tag::TYPE_FRONTPAGE );
		}

		$msg = __( 'Notified LiteSpeed Web Server to purge the front page.', 'zinn-cache-pro' );
		if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
			Admin_Display::success( $msg );
		}
		do_action( 'zinn_cache_pro_purged_frontpage' );
	}
	...
	private function _purge_pages() {
		...
		$msg = __( 'Notified LiteSpeed Web Server to purge all pages.', 'zinn-cache-pro' );
		if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
			Admin_Display::success( $msg );
		}
		do_action( 'zinn_cache_pro_purged_pages' );
	}
	...
	private function _purge_error( $type = false ) {
		...
		$msg = __( 'Notified LiteSpeed Web Server to purge error pages.', 'zinn-cache-pro' );
		if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
			Admin_Display::success( $msg );
		}
	}
	...
	public function purge_cat( $value ) {
		...
		if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
			Admin_Display::success( sprintf( __( 'Purge category %s', 'zinn-cache-pro' ), $val ) );
		}

		do_action( 'zinn_cache_pro_purged_cat', $value );
	}
	...
	public function purge_tag( $val ) {
		...
		if ( ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
			Admin_Display::success( sprintf( __( 'Purge tag %s', 'zinn-cache-pro' ), $val ) );
		}

		do_action( 'zinn_cache_pro_purged_tag', $val );
	}
	...
	public function purge_url( $url, $purge2 = false, $quite = false ) {
		...
		if ( ! $quite && ! defined( 'ZINN_CACHE_PRO_PURGE_SILENT' ) ) {
			Admin_Display::success( sprintf( __( 'Purge url %s', 'zinn-cache-pro' ), $val ) );
		}

		do_action( 'zinn_cache_pro_purged_link', $url );
	}
	...
	public static function purge_esi( $tag ) {
		...
		do_action( 'zinn_cache_pro_purged_esi', $tag );
	}
	...
	public static function purge_posttype( $post_type ) {
		...
		do_action( 'zinn_cache_pro_purged_posttype', $post_type );
	}
	...
	public function purge_post( $pid ) {
		...
		do_action( 'zinn_cache_pro_purged_post', $pid );
	}
	...
	public static function purge_widget( $widget_id = null ) {
		...
		do_action( 'zinn_cache_pro_purged_widget', $widget_id );
	}
	...
	public static function purge_comment_widget() {
		...
		if ( null !== $recent_comments ) {
			...
			do_action( 'zinn_cache_pro_purged_comment_widget', $recent_comments->id );
		}
	}
	...
	public function purge_feeds() {
		...
		do_action( 'zinn_cache_pro_purged_feeds' );
	}
	...
	public static function purge_on_logout() {
		...
		do_action( 'zinn_cache_pro_purged_on_logout' );
	}
	...
	private function _finalize() {
		if ( ! defined( 'ZINN_CACHE_PRO_DID_' . __FUNCTION__ ) ) {
			define( 'ZINN_CACHE_PRO_DID_' . __FUNCTION__, true );
		} else {
			return;
		}

		do_action( 'zinn_cache_pro_purge_finalize' );
	...
	private function _append_prefix( $purge_tags, $is_private = false ) {
		...
		$purge_tags = apply_filters( 'zinn_cache_pro_purge_tags', $purge_tags, $is_private );
		if ( ! in_array( '*', $purge_tags, true ) ) {
			$tags = [];
			foreach ( $purge_tags as $val ) {
				$tags[] = ZINN_CACHE_PRO_TAG_PREFIX . $val;
			}
			return $tags;
		}
		...
		if ( ( defined( 'ZINN_CACHE_PRO_EMPTYCACHE' ) && ZINN_CACHE_PRO_EMPTYCACHE ) || $is_private ) {
			return [ '*' ];
		}
		...
		if ( is_multisite() && ! $this->_is_subsite_purge() ) {
			...
			foreach ( $blogs as $blog_id ) {
				$tags[] = ZINN_CACHE_PRO_TAG_PREFIX . $blog_id . '_';
			}
			return $tags;
		}

		return [ ZINN_CACHE_PRO_TAG_PREFIX . $curr_bid . '_' ];
	}
	...
	private function _is_subsite_purge() {
		...
		if ( defined( 'ZINN_CACHE_PRO_EMPTYCACHE' ) && ZINN_CACHE_PRO_EMPTYCACHE ) {
			return false;
		}
	...
	private function _get_purge_tags_by_post( $post_id ) {
		...
		do_action( 'zinn_cache_pro_api_purge_post', $post_id );
