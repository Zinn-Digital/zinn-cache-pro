...
class Optimizer extends Root {
	...
	public function html_min( $content, $force_inline_minify = false ) {
		if (!apply_filters('zinn_cache_pro_html_min', true)) {
			Debug2::debug2('[Optmer] html_min bypassed via zinn_cache_pro_html_min filter');
			return $content;
		}
	...
	public function serve( $request_url, $file_type, $minify, $src_list ) {
		...
		if ($file_type == 'css') {
			$content = false;
			if (defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(Base::O_OPTM_UCSS)) {
		...
		} elseif ($file_type == 'css' && apply_filters('zinn_cache_pro_ucss_per_pagetype', false)) {
			$url_tag_for_file = $url_tag = Utility::page_type();
		}

		$static_file = ZINN_CACHE_PRO_STATIC_DIR . $file_path_prefix . $url_tag_for_file . '.' . $file_type;
		...
		$realfile        = ZINN_CACHE_PRO_STATIC_DIR . $final_file_path;
	...
	public function optm_font_face( $content ) {
		...
		$optimize_value = apply_filters( 'zinn_cache_pro_font_optimize_value', 'swap' );
	...
	public function optm_snippet( $content, $file_type, $minify, $src, $media = false ) {
		...
		if ($file_type == 'css') {
			...
			if ((defined('ZINN_CACHE_PRO_GUEST_OPTM') || $this->conf(Base::O_IMG_OPTM_WEBP)) && $this->cls('Media')->webp_support()) {
		...
		// Add filter
		$content = apply_filters('zinn_cache_pro_optm_cssjs', $content, $file_type, $src);
	...
	private function load_cached_file( $url, $file_type ) {
		$file_path_prefix     = $this->_build_filepath_prefix($file_type);
		$folder_name          = ZINN_CACHE_PRO_STATIC_DIR . $file_path_prefix;
