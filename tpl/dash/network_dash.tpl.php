...
<?php foreach ( $cloud_summaries as $home_url => $cloud_summary ) : ?>
	...
	<div class="litespeed-dashboard-header">
		<h3 class="litespeed-dashboard-title">
			<?php echo esc_html( sprintf( __( 'Usage Statistics: %s', 'zinn-cache-pro' ), $home_url ) ); ?>
		</h3>
		<hr>
	</div>
		...
		foreach ( $cat_list as $svc => $svc_title ) :
			...
			<div class="postbox litespeed-postbox">
				<div class="inside">
					...
					<div class="litespeed-flex-container">
						...
						<div>
							<div class="litespeed-dashboard-stats">
								<h3><?php echo esc_html( 'img_optm' === $svc ? __( 'Fast Queue Usage', 'zinn-cache-pro' ) : __( 'Usage', 'zinn-cache-pro' ) ); ?></h3>
								<p>
					...
					<?php if ( 0 < $pag_total ) : ?>
						<p class="litespeed-dashboard-stats-payg" data-balloon-pos="up" aria-label="<?php echo esc_attr__( 'Pay as You Go', 'zinn-cache-pro' ); ?>">
							<?php esc_html_e( 'PAYG Balance', 'zinn-cache-pro' ); ?>: <strong><?php echo esc_html( $pag_bal ); ?></strong>
							<button class="litespeed-info-button" data-balloon-pos="up" aria-label="<?php echo esc_attr( sprintf( __( 'This Month Usage: %s', 'zinn-cache-pro' ), esc_html( $pag_used ) ) ); ?>">
					...
					<?php if ( 'img_optm' === $svc ) : ?>
						<p class="litespeed-dashboard-stats-total">
							...
							<button class="litespeed-info-button" data-balloon-pos="up" aria-label="<?php echo esc_attr__( 'Total images optimized in this month', 'zinn-cache-pro' ); ?>">
								<span class="dashicons dashicons-info"></span>
							</button>
						</p>
						<div class="clear"></div>
