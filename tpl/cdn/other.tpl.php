...
<h3 class="litespeed-title-short">
	<?php esc_html_e( 'CDN Settings', 'zinn-cache-pro' ); ?>
	...
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<div id="zinn_cache_pro_cdn_mapping_div"></div>

				<script type="text/babel">
					ReactDOM.render(
						<CDNMapping list={ <?php echo wp_json_encode( $cdn_mapping ); ?> } />,
						document.getElementById( 'zinn_cache_pro_cdn_mapping_div' )
					);
				</script>

				<div class="litespeed-warning">
					<?php esc_html_e( 'NOTE', 'zinn-cache-pro' ); ?>:
					<?php esc_html_e( 'To randomize CDN hostname, define multiple hostnames for the same resources.', 'zinn-cache-pro' ); ?>
				</div>
				...
				<div class="litespeed-desc">
					...
					<?php
					printf(
						esc_html__( 'Serve all image files through the CDN. This will affect all attachments, HTML %1$s tags, and CSS %2$s attributes.', 'zinn-cache-pro' ),
						'<code>&lt;img</code>',
						'<code>url()</code>'
					);
					?>
					...
					<?php esc_html_e( 'Serve all CSS files through the CDN. This will affect all enqueued WP CSS files.', 'zinn-cache-pro' ); ?>
					<br>
					<b><?php $this->title( Base::CDN_MAPPING_INC_JS ); ?></b>:
					<?php esc_html_e( 'Serve all JavaScript files through the CDN. This will affect all enqueued WP JavaScript files.', 'zinn-cache-pro' ); ?>
					<br>
					<b><?php $this->title( Base::CDN_MAPPING_FILETYPE ); ?></b>:
					<?php esc_html_e( 'Static file type links to be replaced by CDN links.', 'zinn-cache-pro' ); ?>
					<?php Doc::one_per_line(); ?>
					<?php
					printf(
						esc_html__( 'This will affect all tags containing attributes: %s.', 'zinn-cache-pro' ),
						'<code>src=""</code> <code>data-src=""</code> <code>href=""</code>'
					);
					?>
					<?php Doc::learn_more( 'https://docs.litespeedtech.com/lscache/lscwp/cdn/#include-file-types', esc_html__( 'Default value', 'zinn-cache-pro' ) ); ?>
					<br>
					<?php
					printf(
						esc_html__( 'If you turn any of the above settings OFF, please remove the related file types from the %s box.', 'zinn-cache-pro' ),
						'<b>' . esc_html__( 'Include File Types', 'zinn-cache-pro' ) . '</b>'
					);
					?>
		...
		<tr>
			...
			<td>
				...
				<div class="litespeed-desc">
					<?php esc_html_e( 'Specify which HTML element attributes will be replaced with CDN Mapping.', 'zinn-cache-pro' ); ?>
					<?php esc_html_e( 'Only attributes listed here will be replaced.', 'zinn-cache-pro' ); ?>
					<br>
					<?php
					printf(
						esc_html__( 'Use the format %1$s or %2$s (element is optional).', 'zinn-cache-pro' ),
		...
		<tr>
			...
			<td>
				...
				<div class="litespeed-desc">
					<?php
					printf(
						esc_html__( 'Site URL to be served through the CDN. Beginning with %1$s. For example, %2$s.', 'zinn-cache-pro' ),
						'<code>//</code>',
					...
					<?php
					printf(
						esc_html__( 'Wildcard %1$s supported (match zero or more characters). For example, to match %2$s and %3$s, use %4$s.', 'zinn-cache-pro' ),
						'<code>*</code>',
		...
		<tr>
			...
			<td>
				...
				<div class="litespeed-desc">
					<?php esc_html_e( 'Only files within these directories will be pointed to the CDN.', 'zinn-cache-pro' ); ?>
					<?php Doc::one_per_line(); ?>
				</div>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<?php $this->build_textarea( $option_id ); ?>
				<div class="litespeed-desc">
					<?php esc_html_e( 'Paths containing these strings will not be served from the CDN.', 'zinn-cache-pro' ); ?>
					<?php Doc::one_per_line(); ?>
				</div>
			</td>
		</tr>
