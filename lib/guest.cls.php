...
class Guest {
	...
	public function __construct() {
		! defined( 'ZINN_CACHE_PRO_CONTENT_FOLDER' ) && define( 'ZINN_CACHE_PRO_CONTENT_FOLDER', dirname( __DIR__, 3 ) );
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- No WP available
		$this->_conf = file_get_contents( ZINN_CACHE_PRO_CONTENT_FOLDER . '/' . self::CONF_FILE );
		if ( $this->_conf ) {
	...
	private function _load_gm_list( $type ) {
		...
		$files = [
			ZINN_CACHE_PRO_CONTENT_FOLDER . '/litespeed/cloud/' . $filename,
