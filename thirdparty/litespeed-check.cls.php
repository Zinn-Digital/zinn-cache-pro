...
class LiteSpeed_Check {
	 ...
	 * @var string
	 */
	private static $_option = 'thirdparty_zinn_cache_pro_check';
	...
	public static function detect() {
		...
		/**
		 * Check for incompatible plugins when `zinn-cache-pro` is first activated.
		 */
		$plugin = basename(ZINN_CACHE_PRO_DIR) . '/zinn-cache-pro.php';
		register_deactivation_hook($plugin, function ( $network_wide ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
			\ZinnCachePro\Admin_Display::delete_option(self::$_option);
		});
		if (!\ZinnCachePro\Admin_Display::get_option(self::$_option)) {
			self::activated_plugin($plugin, null);
			\ZinnCachePro\Admin_Display::add_option(self::$_option, true);
		}
		...
		if (class_exists('PagespeedNinja')) {
			\ZinnCachePro\Admin_Display::error(
				'<div ' .
					self::$_msg_id .
					'>' .
	...
	private static function update_messages() {
		$messages = \ZinnCachePro\Admin_Display::get_option(\ZinnCachePro\Admin_Display::DB_MSG_PIN, array());
		if (is_array($messages)) {
			foreach ($messages as $index => $message) {
				if (strpos($message, self::$_msg_id) !== false) {
					...
					\ZinnCachePro\Admin_Display::update_option(\ZinnCachePro\Admin_Display::DB_MSG_PIN, $messages);
					break;
				}
			}
		}
