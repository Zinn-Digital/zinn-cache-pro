...
class WooCommerce extends Base {
	...
	/**
	 * Whether ESI is enabled via zinn_cache_pro_esi_status filter.
	 *
	 * @var bool
	 */
	private $esi_enabled;
	...
	public function add_hooks() {
		...
		$this->esi_enabled = (bool) apply_filters('zinn_cache_pro_esi_status', false);

		add_action('zinn_cache_pro_control_finalize', [ $this, 'set_control' ]);
		add_action('zinn_cache_pro_tag_finalize', [ $this, 'set_tag' ]);
		...
		if ( $this->esi_enabled ) {
			if ( function_exists('is_shop') && ! is_shop() ) {
				add_action('zinn_cache_pro_tpl_normal', [ $this, 'set_block_template' ]);
				// No need for add-to-cart button
				// add_action( 'zinn_cache_pro_esi_load-wc-add-to-cart-form', [ $this, 'load_add_to_cart_form_block' ] ) ;

				add_action('zinn_cache_pro_esi_load-storefront-cart-header', [ $this, 'load_cart_header' ]);
				add_action('zinn_cache_pro_esi_load-widget', [ $this, 'register_post_view' ]);
			}

			if ( function_exists('is_product') && is_product() ) {
				add_filter('zinn_cache_pro_esi_params', [ $this, 'add_post_id' ], 10, 2);
			}
		...
		if ( is_admin() ) {
			add_action('zinn_cache_pro_api_purge_post', [ $this, 'backend_purge' ]); // todo
			add_action('delete_term_relationships', [ $this, 'delete_rel' ], 10, 2);
			add_action('zinn_cache_pro_settings_tab', [ $this, 'settings_add_tab' ]);
			add_action('zinn_cache_pro_settings_content', [ $this, 'settings_add_content' ]);
			add_filter('zinn_cache_pro_widget_default_options', [ $this, 'wc_widget_default' ], 10, 2);
		}

		if ( apply_filters('zinn_cache_pro_conf', self::O_CART_VARY) ) {
			add_filter(
				'zinn_cache_pro_vary_cookies',
				/**
	...
	public function purge_esi() {
		do_action('zinn_cache_pro_debug', '3rd woo purge ESI in action: ' . current_filter());
		do_action('zinn_cache_pro_purge_private_esi', 'storefront-cart-header');
	}
	...
	public function purge_private_all() {
		do_action('zinn_cache_pro_purge_private_all');
	}
	...
	public function check_if_need_esi( $template ) {
		if ( $this->vary_needed() ) {
			do_action('zinn_cache_pro_debug', 'API: 3rd woo added ESI');
			add_action('zinn_cache_pro_tpl_normal', [ $this, 'set_swap_header_cart' ]);
		}
	...
	public function vary_maintain( $vary ) {
		if ( $this->vary_needed() ) {
			do_action('zinn_cache_pro_debug', 'API: 3rd woo added vary due to cart not empty');
			$vary['woo_cart'] = 1;
		}
	...
	/**
	 * Hooked to the zinn_cache_pro_is_not_esi_template action.
	...
	public function block_template( $template_name, $template_path, $located, $args ) {
		...
		// echo esc_url( apply_filters( 'zinn_cache_pro_esi_url', 'wc-add-to-cart-form', 'WC_CART_FORM', $params ) );
		// // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Allow basic wrapper markup.
		// echo wp_kses_post( apply_filters( 'zinn_cache_pro_clean_wrapper_begin', '' ) );
	}
	...
	public function end_form( $template_name = '' ) {
		...
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Allow basic wrapper markup.
		echo wp_kses_post( apply_filters( 'zinn_cache_pro_clean_wrapper_end', '' ) );
	...
	public function add_related_tags( $args ) {
		...
		foreach ( $related_posts as $related ) {
			do_action('zinn_cache_pro_tag_add_post', $related);
		}
		return $args;
	}
	...
	public function esi_cart_header() {
		...
		echo apply_filters( 'zinn_cache_pro_esi_url', 'storefront-cart-header', 'STOREFRONT_CART_HEADER' );
	}

	/**
	 * Hooked to the zinn_cache_pro_esi_load-storefront-cart-header action.
	...
	/**
	 * Hooked to the zinn_cache_pro_esi_load-wc-add-to-cart-form action.
	...
	/**
	 * Hooked to the zinn_cache_pro_widget_default_options filter.
	 *
	...
	public function set_tag() {
		...
		if ( $ttl && $ttl < apply_filters( 'zinn_cache_pro_control_ttl', 0 ) ) {
			do_action( 'zinn_cache_pro_control_set_ttl', $ttl, "WooCommerce set scheduled TTL to $ttl" );
		}

		if ( function_exists('is_shop') && is_shop() ) {
			do_action('zinn_cache_pro_tag_add', self::CACHETAG_SHOP);
		}
		...
		while ( isset( $term ) ) {
			do_action( 'zinn_cache_pro_tag_add', self::CACHETAG_TERM . $term->term_id );
	...
	public function set_control( $esi_id ) {
		if ( ! apply_filters( 'zinn_cache_pro_control_cacheable', false ) ) {
			return;
		}
		...
		if ( version_compare( $woocom->version, '1.4.2', '>=' ) ) {
			if ( version_compare( $woocom->version, '3.2.0', '<' ) && defined('DONOTCACHEPAGE') && DONOTCACHEPAGE ) {
				do_action( 'zinn_cache_pro_control_set_nocache', '3rd party woocommerce not cache by constant' );
				return;
				...
				if ( $err ) {
					do_action( 'zinn_cache_pro_control_set_nocache', '3rd party woocommerce not cache due to ' . $err );
					return;
				}
			}
			return;
		...
		if ( in_array( $uri, [ 'cart/', 'checkout/', 'my-account/', 'addons/', 'logout/', 'lost-password/', 'product/' ], true ) ) {
			...
			do_action( 'zinn_cache_pro_control_set_nocache', 'uri in cart/account/user pages' );
			return;
		}
		...
		if ( ! empty( $qs ) && $qs_len >= 12 && 0 === strpos( $qs, 'add-to-cart=' ) ) {
			do_action( 'zinn_cache_pro_control_set_nocache', 'qs contains add-to-cart' );
		}
	}
	...
	public function purge_product_stock_status( $product_id, $stock_status = null, $product = null ) {
		...
		do_action( 'zinn_cache_pro_debug', '[3rd] Woo Purge stock status changed [pid] ' . $product->get_id() . ' [status] ' . $stock_status );
	...
	public function purge_cancelled_order_products( $order_id ) {
		...
		foreach ( $products as $product ) {
			do_action( 'zinn_cache_pro_debug', '[3rd] Woo Purge cancelled order managed stock [pid] ' . $product->get_id() );
	...
	public function purge_product( $product, $stock_status_changed = false ) {
		do_action( 'zinn_cache_pro_debug', '[3rd] Woo Purge [pid] ' . $product->get_id() );

		$do_purge = function ( $action, $debug = '' ) use ( $product, $stock_status_changed ) {
			$config = apply_filters( 'zinn_cache_pro_conf', self::O_UPDATE_INTERVAL );
			if ( is_null( $config ) ) {
			...
			if ( self::O_PQS_CQS === $config ) {
				$action();
				if ( $debug ) {
					do_action( 'zinn_cache_pro_debug', $debug );
				}
			} elseif ( ! $stock_status_changed && self::O_PQS_CS !== $config && $product->is_in_stock() ) {
				do_action( 'zinn_cache_pro_debug', '[3rd] Woo No purge needed [option] ' . $config );
				return false;
		...
		do_action( 'zinn_cache_pro_purge_post', $product->get_id() );

		// Check if is variation, purge stock too #984479.
		if ( $product->is_type( 'variation' ) ) {
			do_action( 'zinn_cache_pro_purge_post', $product->get_parent_id() );
		}
		...
		if ( defined( 'WPML_PLUGIN_BASENAME' ) ) {
			...
			foreach ( $translations as $lang => $translation ) {
				do_action( 'zinn_cache_pro_debug', '[3rd] Woo WPML purge language: ' . $translation->language_code . ' , post ID: ' . $translation->element_id );
				do_action( 'zinn_cache_pro_purge_post', $translation->element_id );
				...
				foreach ( $languages as $language ) {
					...
					$do_purge(
						function () use ( $tr_cat_id ) {
							do_action( 'zinn_cache_pro_purge', self::CACHETAG_TERM . $tr_cat_id );
						},
	...
	public function delete_rel( $post_id, $term_ids ) {
		...
		foreach ( $term_ids as $term_id ) {
			do_action( 'zinn_cache_pro_purge', self::CACHETAG_TERM . $term_id );
		}
	}
	...
	public function backend_purge( $post_id ) {
		...
		if ( ! empty( $cats ) ) {
			foreach ( $cats as $cat ) {
				do_action( 'zinn_cache_pro_purge', self::CACHETAG_TERM . $cat );
			}
		}
		...
		if ( ! empty( $tags ) ) {
			foreach ( $tags as $tag ) {
				do_action( 'zinn_cache_pro_purge', self::CACHETAG_TERM . $tag );
			}
		}
	}
	...
	public function add_review( $unused, $comment_approved, $commentdata ) {
		...
		if ( ! is_null( $recent_reviews ) ) {
			do_action( 'zinn_cache_pro_tag_add_widget', $recent_reviews->id );
		}
	}
	...
	private function _option_append() {
		...
		do_action( 'zinn_cache_pro_conf_multi_switch', self::O_UPDATE_INTERVAL, 3 ); // This need to be before conf_append.

		do_action( 'zinn_cache_pro_conf_append', self::O_UPDATE_INTERVAL, false );
		do_action( 'zinn_cache_pro_conf_append', self::O_CART_VARY, false );
	}

	/**
	 * Hooked to `zinn_cache_pro_settings_tab` action.
	...
	public static function bulk_edit_purge() {
		...
		foreach ( $stock_string_arr as $edited_stock ) {
			...
			if ( empty( $product ) ) {
				do_action( 'zinn_cache_pro_debug', '3rd woo purge: ' . $product_id . ' not found.' );
				continue;
			}
