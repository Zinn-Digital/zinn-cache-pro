...
class Elementor {
	...
	public static function preload() {
		...
		if ( 'elementor' === $action ) {
			do_action( 'zinn_cache_pro_disable_all', 'elementor edit mode' );
		}
		...
		if ( '' !== $http_referer && false !== strpos( $http_referer, 'action=elementor' ) ) {
			...
			do_action( 'zinn_cache_pro_disable_all', 'elementor edit mode in HTTP_REFERER' );
		}

		// Clear LSC cache when Elementor regenerates CSS & Data.
		add_action( 'elementor/core/files/clear_cache', __CLASS__ . '::regenerate_zinn_cache_pro_cache' );
	}
	 ...
	 * @return void
	 */
	public static function regenerate_zinn_cache_pro_cache() {
		do_action( 'zinn_cache_pro_purge_all', 'Elementor - Regenerate CSS & Data' );
	}
}
