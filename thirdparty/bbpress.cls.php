...
class BBPress {
	...
	public static function detect() {
		if ( function_exists( 'is_bbpress' ) ) {
			add_action( 'zinn_cache_pro_api_purge_post', __CLASS__ . '::on_purge' ); // todo
	...
	public static function set_control() {
		...
		if ( is_bbpress() && Router::is_logged_in() ) {
			do_action( 'zinn_cache_pro_control_set_nocache', 'bbpress nocache due to loggedin' );
		}
	}
	...
	public static function on_purge( $post_id ) {
		...
		do_action( 'zinn_cache_pro_purge_posttype', bbp_get_forum_post_type() );
		$ancestors = get_post_ancestors( $post_id );

		// If there are ancestors, need to purge them as well.
		if ( ! empty( $ancestors ) ) {
			foreach ( $ancestors as $ancestor ) {
				do_action( 'zinn_cache_pro_purge_post', $ancestor );
			}
		}
		...
		if ( bbp_is_reply( $post_id ) && $replies_widget ) {
			do_action( 'zinn_cache_pro_purge_widget', $replies_widget->id );
		}

		$topic_widget = $wp_widget_factory->get_widget_object( 'BBP_Topics_Widget' );
		if ( bbp_is_topic( $post_id ) && $topic_widget ) {
			do_action( 'zinn_cache_pro_purge_widget', $topic_widget->id );
		}
	}
}
