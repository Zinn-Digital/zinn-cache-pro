...
class NextGenGallery {
	...
	public static function add_image( $image ) {
		...
		if ($gallery && $gallery->pageid) {
			do_action('zinn_cache_pro_purge', self::CACHETAG_GALLERIES . $gallery->pageid);
		}
	}
	...
	public static function update_image() {
		...
		if ( isset( $_REQUEST['gallery_id'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			do_action( 'zinn_cache_pro_purge', self::CACHETAG_GALLERIES . sanitize_key( wp_unslash( $_REQUEST['gallery_id'] ) ) );
			return;
		}
		...
		if ( isset( $_POST['task_list'] ) ) {
			...
			if ( ! empty( $task_list[0]['query']['id'] ) ) {
				do_action( 'zinn_cache_pro_purge', self::CACHETAG_GALLERIES . sanitize_key( $task_list[0]['query']['id'] ) );
				return;
			}
		}
		...
		if ($image) {
			do_action('zinn_cache_pro_purge', self::CACHETAG_GALLERIES . $image->galleryid);
		}
	}
	...
	public static function delete_image() {
		...
		if ( isset( $_GET['gid'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			do_action( 'zinn_cache_pro_purge', self::CACHETAG_GALLERIES . sanitize_key( wp_unslash( $_GET['gid'] ) ) );
		}
	}
	...
	public static function move_image( $images, $old_gallery_ids, $new_gallery_id ) {
		foreach ($old_gallery_ids as $gid) {
			do_action('zinn_cache_pro_purge', self::CACHETAG_GALLERIES . $gid);
		}
		do_action('zinn_cache_pro_purge', self::CACHETAG_GALLERIES . $new_gallery_id);
	}
	...
	public static function update_album( $aid ) {
		do_action('zinn_cache_pro_purge', self::CACHETAG_ALBUMS . $aid);
	}
	...
	public static function add_container( $render_parms ) {
		...
		foreach ($container_ids as $id) {
			do_action('zinn_cache_pro_tag_add', $tag . $id);
		}
