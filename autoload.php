...
$zinn_cache_pro_php_files = array(
	// core file priority
	'src/root.cls.php',
	'src/base.cls.php',
...
foreach ($zinn_cache_pro_php_files as $class) {
	$file = ZINN_CACHE_PRO_DIR . $class;
	require_once $file;
}

if (!function_exists('zinn_cache_pro_autoload')) {
	/**
	 ...
	 */
	function zinn_cache_pro_autoload( $cls ) {
		if (strpos($cls, '.') !== false) {
			return;
		}

		if (strpos($cls, 'ZinnCachePro') !== 0) {
			return;
		}
		...
		// $file = ZINN_CACHE_PRO_DIR . $file . '.cls.php';
		// } else {
		// $file = ZINN_CACHE_PRO_DIR . 'src/' . $file . '.cls.php';
		// }
		// phpcs:enable
...
spl_autoload_register('zinn_cache_pro_autoload');
